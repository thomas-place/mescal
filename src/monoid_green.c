...
{
    // On commence par calculer des Cayley droits et gauche du sous-monoide N
    // (en
        ...
        if (!done[lefread_vertices(G->LCL->cl[c], 0)])
        {
            ...
            for (uint j = 0; j < size_vertices(G->LCL->cl[c]); j++)
            {
                ...
                if (!done[lefread_vertices(G->LCL->cl[c], j)])
                {
                    ...
                    // On donne le numéro de J-classe courant à tous les
                    // éléments de cette
...
void h_green_compute(p_green GREL)
{
    ...
    if (GREL->HCL == NULL)
    {
        ...
        for (uint cr = 0; cr < GREL->JCL->size_par; cr++)
        {
            ...
            // On trie les éléments selon leut numéro de R-classe, puis de
            // L-classe
...
void print_jclass_green(p_cayley M, p_green G, uint jclass, uint padding, FILE *out)
{
    ...
    quick_sort_green(thejclass, size_vertices(G->JCL->cl[G->JCL->numcl[elem]]), G->RCL->numcl,
                     G->LCL->numcl);
        ...
        for (uint k = 0; k < height_hclass; k++)
        {
            ...
            for (uint i = 0; i < row_length; i++)
            {
                ...
                for (uint h = 0; h < num_elem_line; h++)
                {
                    ...
                    if (count < size_hc)
                    {
                        cayley_print_name_aligned(M, thejclass[(j * size_rc) + i * size_hc + count],
                                                  name_size, out);
                    }
                    else
                    {
...
void view_morphism(cayley *thecayley, green *G)
{
    fprintf(stdout, "#### Alphabet  : ");
    shell_print_alphabet(thecayley->trans->size_alpha);
    fprintf(stdout, "#### Monoid    :\n");
    print_full_green(thecayley, G, false, stdout);
    fprintf(stdout, "#### Morphism  :\n");
    cayley_print_morphism(thecayley, stdout);
}
        ...
        for (uint k = 0; k < height_hclass; k++)
        {
            ...
            for (uint i = 0; i < row_length; i++)
            {
                ...
                for (uint h = 0; h < num_elem_line; h++)
                {
                    ...
                    if (count < size_hc)
                    {
                        cayley_print_name_aligned(M,
                                                  S->sub_to_mono[thejclass[(j * size_rc) + i *
                                                                           size_hc + count]],
                                                  name_size, out);
                    }
                    else
