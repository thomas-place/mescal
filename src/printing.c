...
void simple_gedges_print(p_graph G, FILE *out) {
  ...
  for (uint i = 0; i < size_stack(theedges);
       i++) { // Boucle sur les états de départ
...
void simple_lgedges_print(p_stack theedges, FILE *out) {
  for (uint i = 0; i < size_stack(theedges);
    ...
    fprintf(out, "\"%d\" -> \"%d\" [label = <",
            ((p_multi_edge)read_stack(theedges, i))->in,
    ...
    if (!isempty_vertices(((p_multi_edge)read_stack(theedges, i))->lab)) {
      // printf("normal\n");
      for (uint j = 0;
           j < size_vertices(((p_multi_edge)read_stack(theedges, i))->lab) - 1;
           j++) {
        fprintf(
            out, "%c,",
            lefread_vertices(((p_multi_edge)read_stack(theedges, i))->lab, j) +
                'a');
      }
      if (!isempty_vertices(((p_multi_edge)read_stack(theedges, i))->lab_i)) {
        fprintf(
            out, "%c,",
            lefread_vertices(
                ((p_multi_edge)read_stack(theedges, i))->lab,
                size_vertices(((p_multi_edge)read_stack(theedges, i))->lab) -
                    1) +
                'a');
      } else {
        fprintf(
    ...
    if (!isempty_vertices(((p_multi_edge)read_stack(theedges, i))->lab_i)) {
      ...
      for (uint j = 0;
           j <
           size_vertices(((p_multi_edge)read_stack(theedges, i))->lab_i) - 1;
           j++) {
        // printf("j: %d\n", j);
      ...
      fprintf(
          out, "%c<SUP>-1</SUP>>];\n",
          lefread_vertices(
              ((p_multi_edge)read_stack(theedges, i))->lab_i,
              size_vertices(((p_multi_edge)read_stack(theedges, i))->lab_i) -
                  1) +
              'a');
    }
  }
...
void named_dgedges_print(p_dgraph G, void **names, void (*f)(void *, FILE *),
  ...
  for (uint i = 0; i < size_stack(theedges);
    ...
    fprintf(
        ...
        lefread_vertices(
            ((p_multi_edge)read_stack(theedges, i))->lab,
            size_vertices(((p_multi_edge)read_stack(theedges, i))->lab) - 1) +
            'a');
  }
}
...
static void sort_spe_states(p_nfa A, p_vertices inionly, p_vertices finonly,
                            p_vertices mixed) {
  uint i = 0;
  uint j = 0;
  while (i < size_vertices(A->initials) || j < size_vertices(A->finals)) {
    ...
    if (j >= size_vertices(A->finals) ||
        (i < size_vertices(A->initials) &&
         (lefread_vertices(A->initials, i) < lefread_vertices(A->finals, j)))) {
      // printf("Test %d\n", lefread_vertices(A->initials, i));
      rigins_vertices(lefread_vertices(A->initials, i), inionly);
      i++;
    } else if (i < size_vertices(A->initials) &&
               lefread_vertices(A->initials, i) ==
...
void nfa_print(p_nfa A, FILE *out) {
  ...
  if (!isempty_vertices(inionly)) {
    fprintf(out,
            "node [fillcolor=\"blue:green\",style=filled,shape = circle];");
  ...
  if (!isempty_vertices(mixed)) {
    fprintf(
        out,
        "node [fillcolor=\"blue:green\",style=filled,shape = doublecircle];");
