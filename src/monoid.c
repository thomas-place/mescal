...
static uint cayley_get_name_length(p_cayley M, uint q) {
  ...
  uint l = 0;
  // Un entier temporaire pour stocker le nombre de copies consécutives d'une même lettre
  uint num;
  for (uint i = 0;i < size_vertices(M->names[q]);i++) {
    ...
    if (i == 0 || lefread_vertices(M->names[q], i - 1) != lefread_vertices(M->names[q], i)) {
      uint d = 0;
      //On compte le nombre de chiffres du nombre de copies de la lettre précédente
      if (num > 1) {
        while (num != 0) {
          d++;
          num = num / 10;
      ...
      // La longueur est augmentée de ce nombre de chiffre plus la nouvelle lettre lue
      l = l + d + 1;
...
// // On utilise exactement size caractère en ajoutant des espaces (size doit être
// // suffisament grand)
...
void print_sub_cayley_aligned(p_cayley M, p_vertices elems, uint length,
  ...
  for (uint i = 0; i < size_vertices(elems); i++) {
    ...
    if (size_elem == 0) { // Si on a à faire au mot vide, on l'affichera comme 1
      size_elem++;
    }
...
// Fonction auxiliaire
static void cayley_avl_to_table(p_avlnode tree, p_nfa A, p_cayley graph,
  bool* newfinals, bool* theidems) {
  if (tree == NULL) {
    return;
  }
  else {
    cayley_avl_to_table(tree->left, A, graph, newfinals, theidems);
    cayley_avl_to_table(tree->right, A, graph, newfinals, theidems);
    p_cayley_state thestate = (p_cayley_state)tree->value;

    // Affectation des transitions
    for (uint a = 0; a < graph->trans->size_alpha; a++) {
      graph->trans->edges[thestate->num][a] = thestate->next[a]->num;
    }
    ...
    graph->names[thestate->num] = thestate->name;
  }
}
...
p_cayley dfa_to_right_cayley(p_nfa A) {
  ...
  neutral->num = num;
  num++;
  neutral->name = create_vertices(); // Le nom de cet élément est le mot vide
  neutral->size_alpha = A->trans->size_alpha;
  neutral->size_graph = A->trans->size_graph;
  neutral->next = NULL; // Les élément suivants ne sont pas encore calculés
  ...
  thetree = avl_insert(neutral, thetree,
    &comp_cayley_state); // Insertion dans l'ensemble des
  ...
  while (!isempty_dequeue(thequeue)) {
    ...
    for (uint a = 0; a < theelem->size_alpha; a++) {
      // On crée le (potentiel) nouvel élément obtenu en multipliant a à droite
      p_cayley_state new;
      MALLOC(new, 1);
      new->size_graph = A->trans->size_graph;
      MALLOC(new->state, new->size_graph);
      for (uint q = 0; q < neutral->size_graph;
        q++) { // Calcul de la permutation correspondante
        new->state[q] =
      ...
      if (thenode == NULL) { // Si cet état n'a pas encore été construit
        ...
        new->next = NULL; // On ne connaît pas encore les élément suivants
        new->num = num;
        num++;
        new->name = create_vertices(); // Création du nom à partir de celui de
        // l'élément précédent
        copy_vertices_right(new->name, theelem->name, 0);
        rigins_vertices(a, new->name);
        theelem->next[a] =
          new; // On l'affecte comme successeur pour la lettre a
        thetree = avl_insert(
          new, thetree, &comp_cayley_state); // On le mémorise dans l'ensemble
        // des éléments rencontrés
        leftinsert(new, thequeue); // Ce nouvel état élément est à traiter
      }
      else {                     // Sinon, cet état a déjà été construit
        // printf("not NULL\n");
        theelem->next[a] =
          ((p_cayley_state)
            thenode->value); // On affecte la version déjà construite comme
        // successeur pour la lettre a
        free(new->state); // On supprime la copie qu'on vient de créer
        free(new);
      }
    }
  }
  ...
  M->mult = NULL; // Initialement, on ne crée pas la table de multiplication
  M->leftgraph = NULL; // Initialement, on ne crée pas le graphe gauche
  M->order = NULL;     // Initialement, pas d'ordre calculé
  MALLOC(M->accept_array, num); // Tableau de l'ensemble acceptant
  MALLOC(M->idem_array, num);   // Tableau des idempotents
...
void compute_left_cayley(p_cayley M) {
  if (M->leftgraph != NULL) { // Si le calcul a déjà été fait
    return;
  }
      ...
      // identifiants les acceptants
    {
      if (size_vertices(M->accept_list) <= j ||
        ...
        set_nth_bit(&thepairs[r][lefread_vertices(M->accept_list, i)], 1,
          3); // On marque la paire comme ayant été visitée pour le
        // DF1 (bit 3)
        ...
        set_nth_bit(&thepairs[lefread_vertices(M->accept_list, i)][r], 1,
          4); // On marque la paire comme ayant été visitée pour le
        // DF2 (bit 4)
