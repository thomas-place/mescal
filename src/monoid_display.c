...
void mor_print_order(morphism *M, FILE *out)
{
    ...
    for (uint q = 0; q < M->r_cayley->size_graph; q++)
    {
        ...
        mor_print_sub_aligned(M, temp, 100, padding, out);
        delete_dequeue(temp);
    }
    print_bot_line(100, out);

    graph *tred = tclos_reduction_graph(M->order, M->r_cayley->size_graph);
    view_mor_order(M, tred);
    delete_graph(tred);
}
