...
void green_compute_sub_reg(subsemi* S) {
    ...
    for (uint subs = 0; subs < S->size; subs++) {
        ...
        /* printf("e: ");
         mor_fprint_name_utf8(S->original, e, stdout);
         printf("\n");
         printf("f: ");
         mor_fprint_name_utf8(S->original, f, stdout);
         printf("\n");*/
        ...
        for (uint i = 0; i < size_dequeue(G->RCL->cl[G->RCL->numcl[e]]); i++) {
            ...
            if (S->mono_in_sub[s]) {
                rigins_dequeue(S->mono_to_sub[s], re);
            }
        }
        ...
        for (uint i = 0; i < size_dequeue(G->RCL->cl[G->RCL->numcl[f]]); i++) {
            ...
            if (S->mono_in_sub[s]) {
                rigins_dequeue(S->mono_to_sub[s], rf);
            }
        }
        ...
        for (uint i = 0; i < size_dequeue(G->LCL->cl[G->LCL->numcl[e]]); i++) {
            ...
            if (S->mono_in_sub[s]) {
                rigins_dequeue(S->mono_to_sub[s], le);
            }
        }
        // The L-class of e restricted to the elements in the subsemigroup
        ...
        for (uint i = 0; i < size_dequeue(G->LCL->cl[G->LCL->numcl[f]]); i++) {
            ...
            if (S->mono_in_sub[s]) {
                rigins_dequeue(S->mono_to_sub[s], lf);
            }
        }
        ...
        /* printf("reilf: ");
         for (uint k = 0; k < size_dequeue(reilf); k++) {
             uint t = lefread_dequeue(reilf, k);
             mor_fprint_name_utf8(S->original, S->sub_to_mono[t], stdout);
             printf(" ");
         }
         printf("\n"); */
        ...
        /*  printf("leirf: ");
          for (uint k = 0; k < size_dequeue(leirf); k++) {
              uint t = lefread_dequeue(leirf, k);
              mor_fprint_name_utf8(S->original, S->sub_to_mono[t], stdout);
              printf(" ");
          }
          printf("\n");
