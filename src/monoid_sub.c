...
void green_compute_sub(subsemi *S)
{
    ...
    dyn_edge_pair *cayr_edges = create_dyn_edge_pair();
    dyn_edge_pair *cayl_edges = create_dyn_edge_pair();
    dyn_edge_pair *cayj_edges = create_dyn_edge_pair();
    ...
    for (uint q = 0; q < S->size; q++)
    {
        ...
        {
            int rprod = mor_mult(S->original, S->sub_to_mono[q], S->sub_to_mono[r]);
            int lprod = mor_mult(S->original, S->sub_to_mono[r], S->sub_to_mono[q]);

            dyn_edge_pair_add(cayr_edges, q, S->mono_to_sub[rprod]);
            dyn_edge_pair_add(cayl_edges, q, S->mono_to_sub[lprod]);
            dyn_edge_pair_add(cayj_edges, q, S->mono_to_sub[rprod]);
            dyn_edge_pair_add(cayj_edges, q, S->mono_to_sub[lprod]);
    ...
    graph *cayr = edge_list_to_graph(cayr_edges->array, S->size, cayr_edges->size);
    graph *cayl = edge_list_to_graph(cayl_edges->array, S->size, cayl_edges->size);
    graph *cayj = edge_list_to_graph(cayj_edges->array, S->size, cayj_edges->size);
    delete_dyn_edge_pair(cayr_edges);
    delete_dyn_edge_pair(cayl_edges);
    delete_dyn_edge_pair(cayj_edges);
