...
static dgraph *morphism_to_cut_kernel(morphism *M)
{
    dgraph *g = create_dgraph_noedges(M->r_cayley->size_graph, M->r_cayley->size_alpha);
    for (uint q = 0; q < M->r_cayley->size_graph; q++)
    {
        for (uint a = 0; a < M->r_cayley->size_alpha; a++)
        {
            if (M->rels->RCL->numcl[q] == M->rels->RCL->numcl[M->r_cayley->edges[q][a]] || !M->rels->regular_array[M->r_cayley->edges[q][a]])
            {
                g->edges[q][a] = M->r_cayley->edges[q][a];
                g->size_edges++;
            }
        }
    }
...
subsemi *get_kernel(morphism *M, sub_level level, basis ba)
{
    ...
    ker->level = LV_FULL;
    dgraph *cut = morphism_to_cut_kernel(M);
    parti *FOLD;
    lgraph *EXT = dgraph_dyck_ext(cut, ba == BA_GR, NULL, &FOLD);
    delete_dgraph(cut);
    ...
    for (uint k = 0; k < M->nb_idems; k++)
    {
        ...
        uint start = LGSTART(EXT, FOLD->numcl[e], EXT->size_alpha - 1);
        uint end = LGEND(EXT, FOLD->numcl[e], EXT->size_alpha - 1);
