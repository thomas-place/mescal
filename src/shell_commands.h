...
typedef enum {
    MODE_DEFAULT,   //!< Mode par défaut: enregistre un nouvel objet uniquement si il n'existait pas c'est nécessaire.
    MODE_PRINT,     //!< Mode d'affichage: n'enregistre pas de nouvel objet.
    MODE_COPY,      //!< Mode copie: créé un nouvel objet même si il existe déjà (il y a une copie dans ce cas).
} com_mode;
