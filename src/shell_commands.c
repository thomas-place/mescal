...
int com_apply_command(com_command *thecom, char *varname, com_mode mode, bool *saved)
{
  ...
  if (mode == MODE_PRINT)
  {
    ...
    case KY_FACTO:
      return shell_view_facto(thecom->params, thecom->main->string);
      break;
    case KY_GKER:
  ...
  case KY_MIRROR:
    ...
    break;
  case KY_LQUOTIENT:
    k = shell_quotient_nfa(varname, thecom->params, thecom->main->string, true);
    break;
  case KY_RQUOTIENT:
    k = shell_quotient_nfa(varname, thecom->params, thecom->main->string, false);
    break;
  case KY_TRIMNFA:
...
int shell_quotient_nfa(char *varname, com_parameters *pars, const char *str, bool side)
{
  // Retrieval of the parameters (errors are handled in the function).
  char *raw;
  int i;
  ...
  if (param_retrieve(pars, 2, 0, types, NULL, &raw, NULL, &i, &saved, str) == -2)
  {
    // usage_generic(types, 1, PAR_AUTOREG, str);
    return -2;
  }

  word *myword = word_from_string(raw);

  int j;
  if (objects[i].type == DAUTOMATON)
  {
    dfa *automaton = side ? dfa_left_quotient(objects[i].obj_dfa, myword) : dfa_right_quotient(objects[i].obj_dfa, myword);
    j = object_add_automaton_dfa(varname, automaton);
  }
  else if (objects[i].type == NAUTOMATON)
  {
    nfa *automaton = side ? nfa_left_quotient(objects[i].obj_nfa, myword) : nfa_right_quotient(objects[i].obj_nfa, myword);
    j = object_add_automaton_nfa(varname, automaton);
  }
  else
  {
    nfa *automaton = reg_thompson(objects[i].exp);
    nfa *automaton2 = side ? nfa_left_quotient(automaton, myword) : nfa_right_quotient(automaton, myword);
    nfa_delete(automaton);
    j = object_add_automaton_nfa(varname, automaton2);
  }
  delete_word(myword);
  if (saved)
  {
...
int shell_trim_nfa(char *varname, com_parameters *pars, const char *str)
{
  ...
  if (objects[i].type == DAUTOMATON)
  {
    dfa *automaton = dfa_trim(objects[i].obj_dfa, true, false);
    if (saved)
    {
      object_free(i);
    }
  ...
  {
    nfa *automaton = nfa_trim(objects[i].obj_nfa, true);
    if (saved)
    {
      object_free(i);
    }
...
int shell_invtrans(char *varname, com_parameters *pars, const char *str)
{
  ...
  if (objects[i].type == DAUTOMATON)
  {
    ...
    nfa *nauto = nfa_inv_ext(automaton, NULL);
    nfa_delete(automaton);
    automaton = nauto;
  }
  else
  {
    automaton = nfa_inv_ext(objects[i].obj_nfa, NULL);
  }
  if (saved)
  {
    object_free(i);
...
int shell_dycktrans_nfa(char *varname, com_parameters *pars, const char *str)
{
  ...
  if (objects[i].type == DAUTOMATON)
  {
    automaton = dfa_dyck_ext(objects[i].obj_dfa, true, true, NULL, NULL);
  }
  else
  {
    automaton = nfa_dyck_ext(objects[i].obj_nfa, true, true, NULL, NULL);
  }
...
int shell_view_rcayley(com_parameters *pars, const char *str)
{
  ...
  view_cayley(objects[i].mor->obj, false);
  if (saved)
  {
    object_free(i);
  }
...
int shell_view_lcayley(com_parameters *pars, const char *str)
{
  ...
  view_cayley(objects[i].mor->obj, true);
  if (saved)
  {
    object_free(i);
  }
...
int shell_view_morder(com_parameters *pars, const char *str)
{
  ...
  case DAUTOMATON:
    ...
    dfa_mini_canonical_ordering(objects[j].obj_dfa);
    print_title_box(100, true, stdout, 1, "Minimal automaton.");
    shell_view_object(objects + j, false);
    print_title_box(100, true, stdout, 1, "Canonical ordering on the minimal automaton.");
    dfa_print_order(objects[j].obj_dfa, stdout);
    break;
  case MORPHISM:
...
int shell_view_nfa_run(com_parameters *pars, const char *str)
{
  ...
  if (objects[k].type == DAUTOMATON)
  {
    ...
    uint s = dfa_compute_run(A, myword);
    if (s == UINT_MAX - 1)
    {
    ...
    {
      fprintf(stdout, "The word leads to no state.\n");
  ...
  else
  {
    ...
    uint *states = nfa_compute_runs(A, myword);
    if (states == NULL)
    {
      fprintf(stderr, "This word contains letters outside the alphabet of this automaton.\n");
    }
    ...
    {
      fprintf(stdout, "States reached: ");
      bool found = false;
      bool accept = false;
      uint j = 0;
      for (uint q = 0; q < A->trans->size_graph; q++)
      {
        if (states[q / sizeof(uint)] & (1 << (q % sizeof(uint))))
        {
          if (found)
          {
            printf(",");
          }
          nfa_print_state(A, q, stdout);
          found = true;
          if (q == A->finals[j])
          {
            accept = true;
...
int shell_view_mor_image(com_parameters *pars, const char *str)
{
  ...
  uint s = mor_compute_image(M, myword);
...
static void usage_facto(const char *str)
{
  fprintf(stderr, "#### Usage  : %s(<com>, <word>)\n", str);
  fprintf(stderr, "####          <com> is a command evaluating to a morphism.\n");
  fprintf(stderr, "####          <word> is a word.\n");
...
int shell_view_facto(com_parameters *pars, const char *str)
{
  ...
  shell_view_object(objects + k, true);
  if (size_word(myword) == 0)
  {
    fprintf(stderr, "Factorization forests are only computed for non-empty words.\n");
    delete_word(myword);
    if (saved)
    {
      object_free(k);
  ...
  {
    fprintf(stdout, "Computing a factorization forest of the word %s for this morphism.\n", pars->next->param->main->string);
    morphism *M = objects[k].mor->obj;
    facto_forest *F = mor_compute_facto_forest(M, myword);
    if (F == NULL)
    {
      fprintf(stderr, "Error: This is not a valid word for this morphism.\n");
      delete_word(myword);
    ...
    fprintf(stdout, "Factorization forest:\n");
    view_facto_forest(M, F);
    free(F->childrens);
    free(F->nodes);
    free(F);
    delete_word(myword);
...
int shell_counterfree(com_parameters *pars, const char *str)
{
  ...
  int error = 0;
  shell_autoprop_sfgp(i, BA_ST, &error, stdout);
  if (error < 0)
  {
...
int shell_autoda(com_parameters *pars, const char *str)
{
  ...
  shell_autoprop_dagp(i, BA_ST, &error, stdout);
  if (error < 0)
  {
...
int shell_permutation(com_parameters *pars, const char *str)
{
  ...
  int error = 0;
  shell_autoprop_permut(i, &error, stdout);
  if (error < 0)
  {
...
int shell_rtrivialrivial(com_parameters *pars, const char *str)
{
  ...
  int error = 0;
  shell_autoprop_rtrivialgp(i, BA_ST, &error, stdout);
  if (error < 0)
  {
...
int shell_commutative(com_parameters *pars, const char *str)
{
  ...
  case NAUTOMATON:
  case DAUTOMATON:
    shell_autoprop_commut(i, &error, stdout);
    break;
  case MORPHISM:
...
int shell_letteruniform(com_parameters *pars, const char *str)
{
  ...
  case NAUTOMATON:
  case DAUTOMATON:
    shell_autoprop_letterind(i, &error, stdout);
    break;
  case MORPHISM:
...
int shell_separation(com_parameters *pars, const char *str)
{
  ...
  for (uchar i = 0; i < 2; i++)
  {
    ...
    {
    case NAUTOMATON:
      A[i] = nfa_copy(objects[inds[i]].obj_nfa);
      break;
    case DAUTOMATON:
    ...
    case REGEXP:
    {
      nfa *A1 = reg_thompson(objects[inds[i]].exp);
      nfa *A2 = nfa_elimeps(A1);
      A[i] = nfa_trim(A2, false);
      nfa_delete(A1);
      nfa_delete(A2);
