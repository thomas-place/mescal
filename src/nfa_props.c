...
uint *make_singleton_word(uint the_letter) {
    uint *the_word;
    MALLOC(the_word, 2);
    the_word[0] = the_letter;
    the_word[1] = UINT_MAX;
    return the_word;
}
...
uint *copy_word(uint *theword) {
    if (!theword) {
        return NULL;
    }
    uint len = 0;
    while (theword[len] != UINT_MAX) {
        len++;
    }
    uint *newword;
...
bool is_trivial_dfa(dfa *A, FILE *out) {
    if (out) {
        fprintf(out, "#### Checking if this automaton is trivial (i.e., has a "
                     "single state).\n");
    }
...
bool is_ddtrivial_dfa(dfa *A, FILE *out) {
    ...
    // We look for any transition that does not map to the same state (in which
    // case, the pattern is not satisfied).
...
bool is_letterind_dfa(dfa *A, FILE *out) {
    ...
    // We look for any state where two letters have different actions (in which
    // case, the pattern is not satisfied).
    for (uint s = 0; s < A->trans->size_graph; s++) {
        for (uint a = 1; a < A->trans->size_alpha; a++) {
            if (A->trans->edges[s][0] != A->trans->edges[s][a]) {
                if (out) {
                    fprintf(out, "#### The automaton is not letter uniform as "
                                 "it contains the transitions ");
                    fprintf(out, "(");
...
bool is_permutation_dfa(dfa *A, FILE *out) {
    ...
    // For each letter a, we look for two a-transitions leading to the same
    // state.
    for (uint a = 0; a < A->trans->size_alpha; a++) {
        // We remember which states have already been reached by an
        // a-transition.
        for (uint q = 0; q < A->trans->size_graph; q++) {
            reached[q] = UINT_MAX;
        }
    ...
    for (uint a = 0; a < A->trans->size_alpha; a++) {
        ...
        for (uint q = 0; q < A->trans->size_graph; q++) {
            ...
            if (out) {
                ...
                mypattern->words[0]        = make_singleton_word(a);
                dgraph_exists_path(A->trans, A->initial, reached[r], true,
                                   &mypattern->words[1]);
                dgraph_exists_path(A->trans, A->initial, q, true,
                                   &mypattern->words[2]);
...
bool is_comm_dfa(dfa *A, FILE *out) {
    ...
    // We look for two letters a and b and a state q such that the a-transition
    // followed by the b-transition does not lead to the same state as the
    // b-transition followed by the a-transition.
    for (uint a = 0; a < A->trans->size_alpha; a++) {
...
bool is_alphaidem_dfa(dfa *A, FILE *out) {
    ...
    // We look for any state q and letter a such that the a-transition from q
    // followed by another a-transition does not lead to the same state as the
    // single a-transition from q.  This only tests idempotency of each letter
    // (which is equivalent to full idempotency in the commutative case).
    for (uint a = 0; a < A->trans->size_alpha; a++) {
        for (uint q = 0; q < A->trans->size_graph; q++) {
            uint r = A->trans->edges[q][a];
    ...
    for (uint q = 0; q < A->trans->size_graph; q++) {
        for (uint r = 0; r < A->trans->size_graph; r++) {
            ...
            for (uint s = 0; s < A->trans->size_graph; s++) {
                ...
                if (s == r || !dgraph_ntrivial_loop(inter, scci, qs) ||
                    !dgraph_ntrivial_loop(inter, scci, rs)) {
                    continue;
                }
                if (dgraph_exists_intersec_path(inter, inter, qr, rs, qr, rs,
                                                true, NULL) &&
                    dgraph_exists_intersec_path(A->trans, A->trans, q, r, r, s,
                                                true, NULL)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        mypattern->states[0]       = q;
                        ...
                        mypattern->types[0]        = PAT_WORD;
                        mypattern->types[1]        = PAT_WORD;
                        dgraph_exists_intersec_path(A->trans, A->trans, q, r, r,
                                                    s, true,
                                                    &mypattern->words[0]);
                        dgraph_exists_intersec_path(inter, inter, qr, rs, qr,
                                                    rs, true,
                                                    &mypattern->words[1]);
                        view_pattern(A, mypattern, idem_pattern_print);
                        free_generic_pattern(mypattern);
...
static uint *word_from_morphism(morphism *M, uint el) {
    ...
    uint *theword;
    MALLOC(theword, n + 1);
    theword[n] = UINT_MAX; // We add a sentinel value at the end of the word
    t          = el;
    while (t != ONE) {
        n--;
        theword[n] = M->pred_lab[t];
        t          = M->pred_ele[t];
    }
    return theword;
}

static uint state_from_word(dfa *A, uint state, uint *theword) {
    uint i = 0;
    while (theword[i] != UINT_MAX) {
        state = A->trans->edges[state][theword[i]];
        i++;
    }
    return state;
}
    ...
    if (is_com_orbmono(ddorbs, c)) {
        ...
        if (out) {
            ...
            for (uint qi = 0; qi < A->trans->size_graph; qi++) {
                if (funs[A->trans->size_graph * el1 + qi] !=
                    funs[A->trans->size_graph * el2 + qi]) {
                    q0 = qi;
                    break;
                }
            }
...
bool is_lt_opti_dfa(dfa *A) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        // If the SCC is trivial, it cannot yield a counterexample and we skip
        // it.
        ...
        // We compute the SCCs of the product graph (used to compute non-trivial
        // loops).
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            // If q and r are distinct and there is a non-trivial common loop on
            // q and r, then we have found a counterexample.
    ...
    while (!isempty_dequeue(queue)) {
        ...
        for (uint a = 0; a < A->trans->size_alpha; a++) {
            ...
            bool stayq = adj_matrix[A->sccs->numcl[qs] * A->sccs->size_par +
                                    A->sccs->numcl[r]];
...
bool is_lttcomm_dfa(dfa *A, int *error, FILE *out) {
    ...
    if (is_comm_ltt_mono(ddorbs, c)) {
        ...
        if (out) {
            ...
            uint el1 =
                mor_mult_gen(M, 7, c[3], c[0], c[4], c[1], c[3], c[2], c[4]);
            uint el2 =
                mor_mult_gen(M, 7, c[3], c[2], c[4], c[1], c[3], c[0], c[4]);
...
// Generates a DFA from a single SCC of an input DFA. A sink state is added to
// the end of the transition graph.
static dfa *dfa_from_scc(dfa *A, parti *sccs, uint p) {
    dfa *D;
    CALLOC(D, 1);
    D->alphabet = duplicate_alphabet(A->alphabet, A->trans->size_alpha);
    D->trans =
        create_dgraph_noedges(sccs->cl_size[p] + 1, A->trans->size_alpha);
    D->nb_finals = 0;
    ...
    for (uint i = 0; i < sccs->cl_size[p]; i++) {
        ...
        for (uint a = 0; a < A->trans->size_alpha; a++) {
            ...
            if (sccs->numcl[r] != p) {
                ...
                D->trans->edges[i][a] =
                    ((uint *)bsearch(&r, sccs->cl_elems[p], sccs->cl_size[p],
...
bool is_counterfreegp_dfa(dfa *A, basis mode, int *error, FILE *out) {
    ...
    for (uint c = 0; c < A->sccs->size_par; c++) {
        // If the SCC consists of a single state, it cannot contain a counter
        // and we skip it.
        ...
        // We compute the associated morphism and save the function table used
        // to construct it.
        *error = 0;
        ...
        // We compute the G-lernel if the basis G is not ST (otherwise, the
        // ST-kernel is the whole monoid and we can directly use the Green
        // relations of the monoid).
        subsemi *ker = mode == BA_ST ? NULL : get_kernel(M, LV_REG, mode);
        green *GREL  = mode == BA_ST ? M->rels : ker->rels;

        // If there exists a non-trivial group, then the automaton contains a
        // G-counter.
        if (GREL->HCL->size_set != GREL->HCL->size_par) {
            ...
            if (out) {
                ...
                // We look for a non-trivial group in the G-kernel and pick a
                // non-idempotent element of this group.
                for (uint i = 0; i < M->nb_idems; i++) {
                    e         = M->idem_list[i];
                    uint sube = ker ? ker->mono_to_sub[e] : e;
                    uint cl   = GREL->HCL->numcl[sube];
                    if (GREL->HCL->cl_size[cl] > 1) {
                        // We take an element of the class which is not an
                        // idempotent.
                ...
                for (uint j = 0; j < D->trans->size_graph; j++) {
                    if (funs[D->trans->size_graph * e + j] == j &&
                        funs[D->trans->size_graph * s + j] != j) {
                        q = A->sccs->cl_elems[c][j];
                        break;
                    }
                }

                // The counter starts at state q, and we have the word w that
                // leads to the counter. We now compute the length of the
                // counter.
                uint n = 0;
                ...
                view_sfc_pattern(A, states, n + 1, mode == BA_ST ? w : NULL,
                                 mode == BA_ST ? 'w' : 'W');
                free(w);
                CONCLUSION_NEGATIVE(out);
            }
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint q = 0; q < thescc->size_graph; q++) {
            for (uint r = q + 1; r < thescc->size_graph; r++) {
                if (scci->numcl[q + r * thescc->size_graph] ==
                    scci->numcl[r + q * thescc->size_graph]) {
                    delete_dgraph(thescc);
                    delete_dgraph(inter);
                    delete_parti(scci);
                    return false;
...
bool is_nosimple_counter_dfa(dfa *A, int *) {
    ...
    for (uint a = 0; a < A->trans->size_alpha; a++) {
        ...
        if (sccs->size_par != sccs->size_set) {
            // If the number of SCCs is not equal to the number of states, then
            // there is a counter.
            delete_parti(sccs);
            return false;
        }
...
bool is_rtrivialgp_dfa_noprint(dfa *A, basis mode, generic_pattern *cexa) {
            ...
            if (cexa) {
                for (uint i = 0; i < A->sccs->size_par; i++) {
                    if (A->sccs->cl_size[i] > 1) {
                        ...
                        dgraph_exists_path(A->trans, cexa->states[0],
                                           cexa->states[1], false,
                                           &cexa->words[0]);
                        dgraph_exists_path(A->trans, cexa->states[1],
                                           cexa->states[0], false,
                                           &cexa->words[1]);
                        break;
                    }
    ...
    if (fold->size_par != fold->size_set) {
        // If a counterexample is requested, we return two states from a folded
        // class.
        if (cexa) {
...
bool is_rtrivialgpplus_dfa_noprint(dfa *A, basis mode, generic_pattern *cexa) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        parti *foldscc =
            mode == BA_ST ? NULL : dgraph_stal_fold(thescc, NULL, mode);
        // We compute the direct product of the SCC with itself.
        dgraph *product = dgraph_direct_product(thescc, thescc);
        // We compute the SCCs of the product graph (used to compute non-trivial
        // loops).
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            if (q != r &&
                (!foldscc || foldscc->numcl[q] == foldscc->numcl[r]) &&
                dgraph_ntrivial_loop(product, sccp, qr)) {
                if (cexa) {
...
bool is_rtrivialbpgp_dfa_noprint(dfa *A, basis mode, generic_pattern *cexa) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i = 0; i < A->sccs->size_par; i++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                if (dgraph_exists_path_alpha(thescc, q, r, false, alpha,
                                             NULL) &&
                    dgraph_exists_path_alpha(thescc, r, q, false, alpha,
                                             NULL)) {
                    if (cexa) {
                        cexa->states[0] = A->sccs->cl_elems[i][q];
                        cexa->states[1] = A->sccs->cl_elems[i][r];
                        ...
                        cexa->types[1]  = PAT_WORD;
                        cexa->types[2]  = PAT_WORD;
                        dgraph_exists_path_alpha(thescc, q, r, false, alpha,
                                                 &cexa->words[0]);
                        dgraph_exists_path_alpha(thescc, r, q, false, alpha,
                                                 &cexa->words[1]);
                        dgraph_getloop_alpha(product, sccp, alpha, qr,
                                             &cexa->words[2]);
                    }
                    delete_dgraph(product);
                    delete_parti(sccp);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            if (dgraph_exists_intersec_path(g_fold, thescc, fold->numcl[qr], q,
                                            fold->numcl[qr], r, false, NULL) &&
                dgraph_exists_intersec_path(g_fold, thescc, fold->numcl[qr], r,
                                            fold->numcl[qr], q, false, NULL)) {
                if (cexa) {
                    cexa->states[0] = A->sccs->cl_elems[i][q];
                    cexa->states[1] = A->sccs->cl_elems[i][r];
                    cexa->vars[0]   = 'U';
                    ...
                    dgraph_exists_intersec_path(g_fold, thescc, fold->numcl[qr],
                                                q, fold->numcl[qr], r, false,
                                                &cexa->words[0]);
                    dgraph_exists_intersec_path(g_fold, thescc, fold->numcl[qr],
                                                r, fold->numcl[qr], q, false,
                                                &cexa->words[1]);
                }
                delete_dgraph(thescc);
                delete_dgraph(product);
...
bool is_rtrivialbpgpplus_dfa_noprint(dfa *A, basis mode,
                                     generic_pattern *cexa) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        // We extract the SCC from the graph.
        dgraph *thescc  = dgraph_extract(A->trans, A->sccs, A->sccs_inv, i);
        // We compute the direct product of the SCC with itself and its own SCCs
        // (used to check synchronized self-loops).
        dgraph *product = dgraph_direct_product(thescc, thescc);
        parti *sccp     = dtarjan(product, NULL, false);
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            // We now need to check the non-separability conditions between q,r
            // and qr
            ...
            parti *theloop_fold =
                mode == BA_ST ? NULL : dgraph_stal_fold(theloop, NULL, mode);

            // If we found a counter-example
            if (dgraph_exists_intersec_path_pgplus(
                    thescc, theloop, q, 0, r, 0, false, theloopprod,
                    scc_loopprod, theloop_fold) &&
                dgraph_exists_intersec_path_pgplus(
                    thescc, theloop, r, 0, q, 0, false, theloopprod,
                    scc_loopprod, theloop_fold)) {
                if (cexa) {
                    cexa->states[0] = A->sccs->cl_elems[i][q];
                    cexa->states[1] = A->sccs->cl_elems[i][r];
            ...
            for (uint h = j; h < end; h++) {
                ...
                for (uint k = 0; k < fold_inter->size_par; k++) {
                    ...
                    for (uint s = 0; s < fold_inter->cl_size[k]; s++) {
                        ...
                        uint x2 =
                            A->sccs->cl_elems[i1][x2x3 / scc2->size_graph];
                        uint x3 =
                            A->sccs->cl_elems[i2][x2x3 % scc2->size_graph];
                        for (uint t = 0; t < fold_inter->cl_size[k]; t++) {
                            uint x1x4 = fold_inter->cl_elems[k][t];
                            uint x1 =
                                A->sccs->cl_elems[i1][x1x4 / scc2->size_graph];
                            uint x4 =
                                A->sccs->cl_elems[i2][x1x4 % scc2->size_graph];
                            for (uint p = 0; p < A->sccs->cl_size[i]; p++) {
                                uint x0 = A->sccs->cl_elems[i][p];

                                // printf("Checking states %u, %u, %u, %u,
                                // %u\n", x0, x1, x2, x3, x4);
                                if (dgraph_exists_intersec_path(
                                        A->trans, inter, x0, x2x3, x1, x1x4,
                                        false, NULL) &&
                                    dgraph_exists_intersec_path(
                                        A->trans, inter, x0, x1x4, x3, x2x3,
                                        false, NULL)) {
                                    if (out) {
                                        generic_pattern *mypattern =
                                            make_generic_pattern();
                                        ...
                                        mypattern->vars[1] =
                                            mode == BA_ST ? 'v' : 'V';
                                        mypattern->types[0] =
                                            mode == BA_ST ? PAT_WORD : PAT_LANG;
                                        mypattern->types[1] =
                                            mode == BA_ST ? PAT_WORD : PAT_LANG;
                                        dgraph_exists_intersec_path(
                                            A->trans, inter, x0, x2x3, x1, x1x4,
                                            false, &mypattern->words[0]);
                                        dgraph_exists_intersec_path(
                                            A->trans, inter, x0, x1x4, x3, x2x3,
                                            false, &mypattern->words[1]);
                                        view_pattern(A, mypattern,
                                                     ltriv_pattern_print);
                                        free_generic_pattern(mypattern);
                                        CONCLUSION_NEGATIVE(out);
                                    }
...
bool is_ltrivialgp_opti_dfa(dfa *A, basis mode, FILE *out) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i = 0; i < scci->size_par; i++) {
            ...
            // if q > r, the state (r,q) is in another scc (rq < qr in this case
            // and the lists of elements are sorted in increasing order). Hence,
            // (q,r) is treated by symmetry with this other scc.
            if (q >= r) {
                continue;
            }
            ...
            for (uint s = 0; s < A->trans->size_graph; s++) {
                if (A->sccs->numcl[q] < A->sccs->numcl[s] ||
                    A->sccs->numcl[r] < A->sccs->numcl[s]) {
                    continue;
                }
                // Check if there is a path from q to r that does not pass
                // through s
                uint *u = NULL;
                uint *v = NULL;
                if (dgraph_exists_path_alpha(A->trans, s, q, false, alpha,
                                             &u) &&
                    dgraph_exists_path_alpha(A->trans, s, r, false, alpha,
                                             &v)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        ...
                        mypattern->words[0]        = u;
                        mypattern->words[1]        = v;
                        dgraph_getloop_alpha(inter, scci, alpha, qr,
                                             &mypattern->words[2]);
    ...
    for (uint i = 0; i < fold->size_par; i++) {
        ...
        // If q = r, this cannot yield a counterexample.
        // if q > r, the state (r,q) is in another scc (rq < qr in this case and
        // the lists of elements are sorted in increasing order). Hence, (q,r)
        // is treated by symmetry with this other scc.
        if (q >= r) {
            continue;
        }
        ...
        for (uint s = 0; s < A->trans->size_graph; s++) {
            ...
            if (dgraph_exists_intersec_path(A->trans, g_fold, s,
                                            fold->numcl[qr], q, fold->numcl[qr],
                                            false, NULL) &&
                dgraph_exists_intersec_path(A->trans, g_fold, s,
                                            fold->numcl[qr], r, fold->numcl[qr],
                                            false, NULL)) {
                if (out) {
                    generic_pattern *mypattern = make_generic_pattern();
                    mypattern->states[0]       = s;
                    ...
                    dgraph_exists_intersec_path(
                        A->trans, g_fold, s, fold->numcl[qr], q,
                        fold->numcl[qr], false, &mypattern->words[0]);
                    dgraph_exists_intersec_path(
                        A->trans, g_fold, s, fold->numcl[qr], r,
                        fold->numcl[qr], false, &mypattern->words[1]);
                    view_pattern(A, mypattern, ltriv_opti_pattern_print);
                    free_generic_pattern(mypattern);
                    CONCLUSION_NEGATIVE(out);
                }
...
bool is_ltrivialgp_rt_dfa(dfa *A, basis mode, FILE *out) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            dgraph *thescc1 =
                dgraph_extract(A->trans, A->sccs, A->sccs_inv, i1);
            for (uint h = j; h < end; h++) {
                ...
                for (uint k = 0; k < sccp->size_par; k++) {
                    ...
                    for (uint x0 = 0; x0 < thescc0->size_graph; x0++) {
                        ...
                        for (uint x1x2 = 0; x1x2 < local->size_graph; x1x2++) {
                            uint x1 =
                                sccp->cl_elems[k][x1x2] / thescc2->size_graph;
                            uint x2 =
                                sccp->cl_elems[k][x1x2] % thescc2->size_graph;
                            uint x1o = A->sccs->cl_elems[i1][x1];
                            uint x2o = A->sccs->cl_elems[i2][x2];
                            if (dgraph_exists_intersec_path(
                                    A->trans, local, x0o, x1x2, x1o, x1x2,
                                    false, NULL) &&
                                dgraph_exists_intersec_path(
                                    A->trans, local, x0o, x1x2, x2o, x1x2,
                                    false, NULL)) {
                                if (out) {
                                    generic_pattern *mypattern =
                                        make_generic_pattern();
                                    ...
                                    if (mode == BA_ST) {
                                        ...
                                        mypattern->types[1] = PAT_WORD;
                                        dgraph_exists_intersec_path(
                                            A->trans, local, x0o, x1x2, x1o,
                                            x1x2, false, &mypattern->words[0]);
                                        dgraph_exists_intersec_path(
                                            A->trans, local, x0o, x1x2, x2o,
                                            x1x2, false, &mypattern->words[1]);
                                    } else {
                                        mypattern->vars[0]  = 'U';
                                        mypattern->vars[1]  = 'V';
        ...
        for (uint j = start; j < end; j++) {
            ...
            for (uint h = j; h < end; h++) {
                ...
                for (uint k = 0; k < scc_inter->size_par; k++) {
                    ...
                    uint r  = qr % scc2->size_graph;
                    if (A->sccs->cl_elems[i1][q] == A->sccs->cl_elems[i2][r] ||
                        !dgraph_ntrivial_scc(inter, scc_inter, k)) {
                        continue;
                    }

                    // Extaction of the SCC of the product graph containing
                    // (q,r)
                    dgraph *thesccp =
                        dgraph_extract(inter, scc_inter, inv_scc_inter, k);

                    // Product of this SCC with itself and its own SCCs (used to
                    // check synchronized self-loops).
                    ...
                    for (uint p = 0; p < sccdblprod->size_par; p++) {
                        ...
                        // Extraction and product with the starting SCC to check
                        // synchronized self-loops.
                        dgraph *thesccdblprod = dgraph_extract(
                            dblprod, sccdblprod, inv_sccdblprod, p);
                        dgraph *ultiprod =
                            dgraph_direct_product(scc0, thesccdblprod);
                        parti *sccultiprod = dtarjan(ultiprod, NULL, false);
                        for (uint m = 0; m < sccultiprod->size_par; m++) {
                            if (!dgraph_ntrivial_scc(ultiprod, sccultiprod,
                                                     m)) {
                                continue;
                            }
                            //
                            uint x0 =
                                A->sccs
                                    ->cl_elems[i][sccultiprod->cl_elems[m][0] /
                                                  thesccdblprod->size_graph];
                            uint quad1 = sccultiprod->cl_elems[m][0] %
                                         thesccdblprod->size_graph;
                            uint quad2 = sccdblprod->cl_elems[p][quad1];
                            uint x2x3 =
                                scc_inter
                                    ->cl_elems[k][quad2 / thesccp->size_graph];
                            uint x1x4 =
                                scc_inter
                                    ->cl_elems[k][quad2 % thesccp->size_graph];
                            uint x2 =
                            ...
                            uint x4 =
                                A->sccs->cl_elems[i2][x1x4 % scc2->size_graph];
                            if ((!fold_inter || fold_inter->numcl[x1x4] ==
                                                    fold_inter->numcl[x2x3]) &&
                                dgraph_exists_intersec_path(A->trans, inter, x0,
                                                            x2x3, x1, x1x4,
                                                            false, NULL) &&
                                dgraph_exists_intersec_path(A->trans, inter, x0,
                                                            x1x4, x3, x2x3,
                                ...
                                if (out) {
                                    ...
                                    dgraph_exists_intersec_path(
                                        A->trans, inter, x0, x2x3, x1, x1x4,
                                        true, &mypattern->words[0]);
                                    dgraph_exists_intersec_path(
                                        A->trans, inter, x0, x1x4, x3, x2x3,
                                        true, &mypattern->words[1]);
                                    dgraph_exists_path(
                                        ultiprod, sccultiprod->cl_elems[m][0],
                                        sccultiprod->cl_elems[m][0], true,
                                        &mypattern->words[2]);
                                    view_pattern(A, mypattern,
                                                 ltriv_pattern_print);
                                    free_generic_pattern(mypattern);
            ...
            for (uint j = start; j < end; j++) {
                ...
                for (uint h = j; h < end; h++) {
                    ...
                    for (uint k = 0; k < scc_inter->size_par; k++) {
                        ...
                        uint q  = qr / scc2->size_graph;
                        uint r  = qr % scc2->size_graph;
                        if (A->sccs->cl_elems[i1][q] ==
                                A->sccs->cl_elems[i2][r] ||
                            !dgraph_ntrivial_scc(inter, scc_inter, k)) {
                            continue;
                        }
                        ...
                        // Product of this SCC with itself and its own SCCs
                        // (used to check synchronized self-loops).
                        dgraph *dblprod =
                            dgraph_direct_product(thesccp, thesccp);
                        parti *sccdblprod    = dtarjan(dblprod, NULL, false);
                        uint *inv_sccdblprod = parti_compute_inv(sccdblprod);
                        for (uint p = 0; p < sccdblprod->size_par; p++) {
                            if (!dgraph_ntrivial_scc(dblprod, sccdblprod, p)) {
                            ...
                            // Extraction and product with the starting SCC to
                            // check synchronized self-loops.
                            dgraph *thesccdblprod = dgraph_extract(
                                dblprod, sccdblprod, inv_sccdblprod, p);
                            dgraph *ultiprod =
                            ...
                            parti *sccultiprod = dtarjan(ultiprod, NULL, false);
                            for (uint m = 0; m < sccultiprod->size_par; m++) {
                                if (!dgraph_compute_alph_scc(
                                        ultiprod, sccultiprod, m, alpha)) {
                                    continue;
                                }
                                //
                                uint x0 =
                                    A->sccs
                                        ->cl_elems[i]
                                                  [sccultiprod->cl_elems[m][0] /
                                                   thesccdblprod->size_graph];
                                uint quad1 = sccultiprod->cl_elems[m][0] %
                                             thesccdblprod->size_graph;
                                uint quad2 = sccdblprod->cl_elems[p][quad1];
                                uint x2x3 =
                                    scc_inter->cl_elems[k][quad2 /
                                                           thesccp->size_graph];
                                uint x1x4 =
                                    scc_inter->cl_elems[k][quad2 %
                                                           thesccp->size_graph];
                                uint x2 =
                                    A->sccs
                                        ->cl_elems[i1][x2x3 / scc2->size_graph];
                                uint x3 =
                                    A->sccs
                                        ->cl_elems[i2][x2x3 % scc2->size_graph];
                                uint x1 =
                                    A->sccs
                                        ->cl_elems[i1][x1x4 / scc2->size_graph];
                                uint x4 =
                                    A->sccs
                                        ->cl_elems[i2][x1x4 % scc2->size_graph];
                                if (dgraph_exists_intersec_path_alpha(
                                        A->trans, inter, x0, x2x3, x1, x1x4,
                                        true, alpha, NULL) &&
                                    ...
                                    if (out) {
                                        ...
                                        dgraph_exists_intersec_path_alpha(
                                            A->trans, inter, x0, x2x3, x1, x1x4,
                                            true, alpha, &mypattern->words[0]);
                                        dgraph_exists_intersec_path_alpha(
                                            A->trans, inter, x0, x1x4, x3, x2x3,
                                            true, alpha, &mypattern->words[1]);
                                        dgraph_getloop_alpha(
                                            ultiprod, sccultiprod, alpha,
                                            sccultiprod->cl_elems[m][0],
                                            &mypattern->words[2]);
                                        view_pattern(A, mypattern,
                                                     ltriv_pattern_print);
                ...
                for (uint k = 0; k < scc_inter->size_par; k++) {
                    ...
                    for (uint p = 0; p < sccdblprod->size_par; p++) {
                        ...
                        for (uint m = 0; m < sccultiprod->size_par; m++) {
                            ...
                            dgraph *myloop = dgraph_extract(
                                ultiprod, sccultiprod, inv_sccultiprod, m);
                            parti *myloop_fold =
                                dgraph_stal_fold(myloop, NULL, mode);
                            dgraph *myloop_fold_red = dgraph_implement_fold(
                                myloop, NULL, myloop_fold);
                            delete_dgraph(myloop);
                            delete_parti(myloop_fold);
                            dgraph *check_prod =
                                dgraph_direct_product(myloop_fold_red, inter);
                            ...
                            // printf("Checking states x0=%u, x1=%u, x2=%u,
                            // x3=%u, x4=%u\n", x0, x1, x2, x3, x4);
                            // view_dgraph(check_prod);

                            if (dgraph_exists_intersec_path(
                                    A->trans, check_prod, x0, x2x3, x1, x1x4,
                                    false, NULL) &&
                                dgraph_exists_intersec_path(
                                    A->trans, check_prod, x0, x1x4, x3, x2x3,
                                    false, NULL)) {
                                if (out) {
                                    generic_pattern *mypattern =
                                        make_generic_pattern();
                                    ...
                                    mypattern->types[2]  = PAT_LANG;
                                    dgraph_exists_intersec_path(
                                        A->trans, check_prod, x0, x2x3, x1,
                                        x1x4, false, &mypattern->words[0]);
                                    dgraph_exists_intersec_path(
                                        A->trans, check_prod, x0, x1x4, x3,
                                        x2x3, false, &mypattern->words[1]);
                                    view_pattern(A, mypattern,
                                                 ltriv_pattern_print);
                                    free_generic_pattern(mypattern);
                                    CONCLUSION_NEGATIVE(out);
...
bool is_ltrivialbpgpplus_dfa(dfa *A, basis mode, int *error, FILE *out) {
    ...
    for (uint i = 0; i < bpolorbs->nb_computed; i++) {
        ...
        if (out) {
            ...
            uint r    = ONE;
            for (uint j = 0;
                 j < S->rels->RCL->cl_size[S->rels->RCL->numcl[esub]]; j++) {
                uint rsub =
                    S->rels->RCL->cl_elems[S->rels->RCL->numcl[esub]][j];
            ...
            uint q0 = ONE;
            for (uint qi = 0; qi < A->trans->size_graph; qi++) {
                if (funs[A->trans->size_graph * e + qi] !=
                    funs[A->trans->size_graph * s + qi]) {
                    q0 = qi;
                    break;
                }
            }
...
static bool is_ppt_dfa(dfa *A, bool copol, FILE *out) {
    ...
    if (out) {
        ...
        if (!is_rtrivialgp_dfa_noprint(A, BA_ST, cexa)) {
            ...
            if (A->order[copol ? cexa->states[1] : cexa->states[0]]
                        [copol ? cexa->states[0] : cexa->states[1]]) {
    ...
    // Computes the canonical ordering of the minimal DFA (does nothing if the
    // ordering is already computed)
    dfa_mini_canonical_ordering(A);
    for (uint q = 0; q < A->trans->size_graph; q++) {
        for (uint a = 0; a < A->trans->size_alpha; a++) {
            // If the pattern equation is satisfied we move to the next
            // transition
            if (A->order[copol ? A->trans->edges[q][a] : q]
                        [copol ? q : A->trans->edges[q][a]]) {
                continue;
            }

            // If the pattern equation is not satisfied, we print a
            // counterexample if requested
            if (out) {
                generic_pattern *mypattern = make_generic_pattern();
                mypattern->states[0]       = q;
...
static bool is_polgr_dfa(dfa *A, bool copol, FILE *out) {
    ...
    for (uint q = 0; q < A->trans->size_graph; q++) {
        for (uint r = 0; r < A->trans->size_graph; r++) {
            ...
            if (dgraph_exists_intersec_path(A->trans, A->trans, q, r, r, r,
                                            false, NULL)) {
                if (out) {
                    generic_pattern *mypattern = make_generic_pattern();
                    mypattern->states[0]       = q;
                    ...
                    mypattern->vars[0]         = 'u';
                    mypattern->types[0]        = PAT_WORD;
                    dgraph_exists_intersec_path(A->trans, A->trans, q, r, r, r,
                                                false, &mypattern->words[0]);
...
bool is_polgpplus_dfa(dfa *A, bool copol, basis mode, FILE *out) {
    if (out) {
        switch (mode) {
        case BA_ST:
            MAKE_PATTERN_INTRO_STRICT(
                copol ? COPOL_PATTERN_ST_PLUS : POL_PATTERN_ST_PLUS, out);
            break;
        case BA_MOD:
            MAKE_PATTERN_INTRO_STRICT(
                copol ? COPOL_PATTERN_MOD_PLUS : POL_PATTERN_MOD_PLUS, out);
            break;
        case BA_GR:
            MAKE_PATTERN_INTRO_STRICT(
                copol ? COPOL_PATTERN_GR_PLUS : POL_PATTERN_GR_PLUS, out);
            break;
        default:
            break;
        }
    ...
    parti *fold =
        mode == BA_ST ? NULL : dgraph_stal_fold(A->trans, A->sccs, mode);
    bool *EXT = mode == BA_ST ? NULL
                              : dgraph_dyck_ext_eps(A->trans, mode == BA_GR,
                                                    A->sccs, fold);
    ...
    // For each pair of SCCs (scc1,scc2) such that there is a path from scc1 to
    // scc2 (checked with the transitive closure)
    for (uint i = 0; i < A->tclos->size; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            // Second scc
            dgraph *scc2    = dgraph_extract(A->trans, A->sccs, A->sccs_inv,
                                             A->tclos->storage[j]);
            ...
            for (uint k = 0; k < sccp->size_par; k++) {
                ...
                // Otherwise, we check the pattern condition, if it fails we
                ...
                if (!A->order[copol ? ro : qo][copol ? qo : ro] &&
                    (!EXT ||
                     EXT[fold->numcl[qo] * fold->size_par + fold->numcl[ro]])) {
                    // If the pattern equation is not satisfied, we print a
                    // counterexample if requested
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        ...
                        mypattern->types[0] =
                            mode == BA_ST ? PAT_WORD : PAT_LANG;
                        mypattern->types[1] = PAT_WORD;
                        dgraph_exists_path(A->trans, qo, ro, true,
                                           &mypattern->words[0]);
                        dgraph_exists_path(product, qr, qr, true,
                                           &mypattern->words[1]);
                        view_pattern(A, mypattern, pol_pattern_print);
                        free_generic_pattern(mypattern);
                        CONCLUSION_NEGATIVE(out);
...
bool is_pol2gp_dfa(dfa *A, bool copol, basis mode, FILE *out) {
    if (out) {
        switch (mode) {
        case BA_ST:
            MAKE_PATTERN_INTRO(copol ? COPOL2_PATTERN_ST : POL2_PATTERN_ST,
                               out);
            break;
        case BA_MOD:
            MAKE_PATTERN_INTRO(copol ? COPOL2_PATTERN_MOD : POL2_PATTERN_MOD,
                               out);
            break;
        case BA_AMT:
            MAKE_PATTERN_INTRO(copol ? COPOL2_PATTERN_AMT : POL2_PATTERN_AMT,
                               out);
            break;
        case BA_GR:
            MAKE_PATTERN_INTRO(copol ? COPOL2_PATTERN_GR : POL2_PATTERN_GR,
                               out);
            break;
        default:
            break;
        ...
        for (uint i = 0; i < A->tclos->size; i++) {
            ...
            for (uint j = start; j < end; j++) {
                ...
                for (uint k = 0; k < sccp->size_par; k++) {
                    ...
                    uint ro = A->sccs->cl_elems[A->tclos->storage[j]][r];
                    if (A->order[copol ? ro : qo][copol ? qo : ro] ||
                        !dgraph_compute_alph_scc(product, sccp, k, calpha)) {
                        continue;
                    }

                    if (dgraph_exists_path_alpha(A->trans, qo, ro, false,
                                                 calpha, NULL)) {
                        if (out) {
                            generic_pattern *mypattern = make_generic_pattern();
                            mypattern->states[0]       = qo;
                            ...
                            mypattern->types[0]        = PAT_WORD;
                            mypattern->types[1]        = PAT_WORD;
                            dgraph_exists_path_alpha(A->trans, qo, ro, false,
                                                     calpha,
                                                     &mypattern->words[0]);
                            dgraph_getloop_alpha(product, sccp, calpha, qr,
                                                 &mypattern->words[1]);
                            view_pattern(A, mypattern, pol_pattern_print);
                            free_generic_pattern(mypattern);
                            CONCLUSION_NEGATIVE(out);
    ...
    for (uint i = 0; i < A->tclos->size; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            for (uint k = 0; k < sccp->size_par; k++) {
                ...
                if (dgraph_exists_intersec_path(g_fold, A->trans, qrc, qo, qrc,
                                                ro, false, NULL)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        mypattern->states[0]       = qo;
                        ...
                        mypattern->types[0]        = PAT_SING;
                        mypattern->types[1]        = PAT_LANG;
                        dgraph_exists_intersec_path(g_fold, A->trans, qrc, qo,
                                                    qrc, ro, false,
                                                    &mypattern->words[0]);
                        view_pattern(A, mypattern, pol_pattern_print);
                        free_generic_pattern(mypattern);
...
bool is_pol2gpplus_dfa(dfa *A, bool copol, basis mode, FILE *out) {
    if (out) {
        ...
        case BA_ST:
            MAKE_PATTERN_INTRO(
                copol ? COPOL2_PATTERN_ST_PLUS : POL2_PATTERN_ST_PLUS, out);
            break;
        case BA_MOD:
            MAKE_PATTERN_INTRO(
                copol ? COPOL2_PATTERN_MOD_PLUS : POL2_PATTERN_MOD_PLUS, out);
            break;
        case BA_AMT:
            MAKE_PATTERN_INTRO(
                copol ? COPOL2_PATTERN_AMT_PLUS : POL2_PATTERN_AMT_PLUS, out);
            break;
        case BA_GR:
            MAKE_PATTERN_INTRO(
                copol ? COPOL2_PATTERN_GR_PLUS : POL2_PATTERN_GR_PLUS, out);
            break;
        default:
            break;
        }
    ...
    for (uint i = 0; i < A->tclos->size; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            for (uint k = 0; k < sccp->size_par; k++) {
                ...
                dgraph *thelooptwo = dgraph_direct_product(theloop, theloop);
                dgraph *theloopprod =
                    dgraph_direct_product(A->trans, thelooptwo);
                parti *scc_loopprod = dtarjan(theloopprod, NULL, false);
                delete_dgraph(thelooptwo);

                parti *fold_loop = mode == BA_ST
                                       ? NULL
                                       : dgraph_stal_fold(theloop, NULL, mode);

                if (dgraph_exists_intersec_path_pgplus(
                        A->trans, theloop, qo, 0, ro, 0, false, theloopprod,
                        scc_loopprod, fold_loop)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        mypattern->states[0]       = qo;
                        mypattern->states[1]       = ro;
...
bool is_dagp_dfa(dfa *A, basis mode, FILE *out) {
    ...
    // We compute the SCCs of the DFA and its transitive closure (to check
    // reachability between SCCs)
    dfa_compute_tclos(A);

    // We loop over the SCCs of the DFA, and for each non-trivial SCC we check
    // the pattern inside it. If a counterexample is found, we return false.
    for (uint i1 = 0; i1 < A->sccs->size_par; i1++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            parti *sccp     = dtarjan(product, NULL, false);
            parti *fold =
                mode == BA_ST ? NULL : dgraph_stal_fold(product, sccp, mode);
            ...
            for (uint k = 0; k < thepar->size_par; k++) {
                ...
                for (uint h1 = 0; h1 < thepar->cl_size[k]; h1++) {
                    ...
                    for (uint h2 = 0; h2 < thepar->cl_size[k]; h2++) {
                        ...
                        if (dgraph_exists_intersec_path(
                                A->trans, product, A->sccs->cl_elems[i1][q0],
                                q1q3, A->sccs->cl_elems[i2][q2], q0q2, false,
                                NULL)) {
                            if (out) {
                                generic_pattern *mypattern =
                                    make_generic_pattern();
                                mypattern->states[0] =
                                    A->sccs->cl_elems[i1][q0];
                                mypattern->states[1] =
                                    A->sccs->cl_elems[i1][q1];
                                mypattern->states[2] =
                                    A->sccs->cl_elems[i2][q2];
                                mypattern->states[3] =
                                    A->sccs->cl_elems[i2][q3];
                                mypattern->vars[0] = mode == BA_ST ? 'u' : 'U';
                                ...
                                mypattern->types[1] =
                                    mode == BA_ST ? PAT_WORD : PAT_LANG;
                                dgraph_exists_path(product, q0q2, q1q3, false,
                                                   &mypattern->words[0]);
                                dgraph_exists_intersec_path(
                                    A->trans, product,
                                    A->sccs->cl_elems[i1][q0], q1q3,
                                    A->sccs->cl_elems[i2][q2], q0q2, false,
                                    &mypattern->words[1]);
...
bool is_dagp_opti_dfa(dfa *A, basis mode, FILE *out) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i = 0; i < A->sccs->size_par; i++) {
            ...
            // Check that no transition labeled by a letter in the alphabet of
            // the SCC goes outside the SCC If so, a contradiction is found
            for (uint k = 0; k < A->sccs->cl_size[i]; k++) {
                uint state = A->sccs->cl_elems[i][k];
                for (uint a = 0; a < A->trans->size_alpha; a++) {
                    if (alphap[a] &&
                        i != A->sccs->numcl[A->trans->edges[state][a]]) {
                        CONCLUSION_NEGATIVE(out);
                        return false;
                    }
                }
            ...
            for (uint l = 0; l < sccp->size_par; l++) {
                ...
                if (q == r ||
                    !dgraph_compute_alph_scc(product, sccp, l, alphal)) {
                    continue;
                }

                if (is_da_compare_alpha(alphap, alphal, A->trans->size_alpha) ||
                    dgraph_exists_path_alpha(thescc, q, r, false, alphal,
                                             NULL)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        ...
                        dgraph_exists_path_alpha(A->trans, qo, ro, false,
                                                 alphal, &mypattern->words[0]);
                        dgraph_getloop_alpha(product, sccp, alphal, qr,
                                             &mypattern->words[1]);
                        view_pattern(A, mypattern, pol_pattern_print);
                        free_generic_pattern(mypattern);
                        CONCLUSION_NEGATIVE(out);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        // We extract the SCC
        dgraph *thescc = dgraph_extract(A->trans, A->sccs, A->sccs_inv, i);
        // We fold the SCC according to the mode (no need to provide the sccs as
        // we are folding only one SCC)
        ...
        for (uint k = 0; k < thescc->size_graph; k++) {
            uint c = fold->numcl[k];
            for (uint a = 0; a < A->trans->size_alpha; a++) {
                if (thescc->edges[k][a] == UINT_MAX &&
                    g_fold->edges[c][a] != UINT_MAX) {
                    delete_dgraph(g_fold);
                    delete_parti(fold);
                    delete_dgraph(thescc);
                    CONCLUSION_NEGATIVE(out);
        ...
        for (uint l = 0; l < sccp->size_par; l++) {
            ...
            if (dgraph_exists_intersec_path(g_foldp, thescc, foldp->numcl[qr],
                                            q, foldp->numcl[qr], r, false,
                                            NULL)) {
                if (out) {
                    generic_pattern *mypattern = make_generic_pattern();
                    mypattern->states[0]       = A->sccs->cl_elems[i][q];
                    ...
                    mypattern->types[1]        = PAT_LANG;
                    dgraph_exists_intersec_path(
                        g_foldp, thescc, foldp->numcl[qr], q, foldp->numcl[qr],
                        r, false, &mypattern->words[0]);
                    view_pattern(A, mypattern, pol_pattern_print);
                    free_generic_pattern(mypattern);
                    CONCLUSION_POSITIVE(out);
                }
    ...
    for (uint i0 = 0; i0 < A->sccs->size_par; i0++) {
        ...
        for (uint n = start; n < end; n++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                if ((i0 == i1 && t1 == t2) ||
                    !dgraph_ntrivial_loop(product, sccp, t)) {
                    continue;
                }
                dgraph *thesccp = dgraph_extract(product, sccp, sccp_inv, j);
                parti *foldp =
                    BA_ST ? NULL : dgraph_stal_fold(thesccp, NULL, mode);
                // view_dgraph(thesccp);
                ...
                for (uint h = 0; h < sccq->size_par; h++) {
                    ...
                    uint x0   = A->sccs->cl_elems[i0][sccp->cl_elems[j][x0x2] /
                                                      thescc1->size_graph];
                    uint x2   = A->sccs->cl_elems[i1][sccp->cl_elems[j][x0x2] %
                                                      thescc1->size_graph];
                    uint x1   = A->sccs->cl_elems[i0][sccp->cl_elems[j][x1x3] /
                                                      thescc1->size_graph];
                    uint x3   = A->sccs->cl_elems[i1][sccp->cl_elems[j][x1x3] %
                                                      thescc1->size_graph];

                    if ((!foldp || foldp->numcl[x0x2] == foldp->numcl[x1x3]) &&
                        dgraph_exists_intersec_path(thesccp, A->trans, x1x3, x0,
                                                    x0x2, x2, true, NULL)) {
                        if (out) {
                            generic_pattern *mypattern = make_generic_pattern();
                            mypattern->states[0]       = x0;
                            ...
                            mypattern->types[1] =
                                mode == BA_ST ? PAT_WORD : PAT_LANG;
                            mypattern->types[2] = PAT_WORD;
                            dgraph_exists_path(thesccp, x0x2, x1x3, true,
                                               &mypattern->words[0]);
                            dgraph_exists_intersec_path(thesccp, A->trans, x1x3,
                                                        x0, x0x2, x2, true,
                                                        &mypattern->words[1]);
                            dgraph_exists_path(quad, qstate, qstate, true,
                                               &mypattern->words[2]);
                            view_pattern(A, mypattern, da_pattern_print);
                            free_generic_pattern(mypattern);
                            CONCLUSION_NEGATIVE(out);
...
bool is_dagpplus_opti_dfa(dfa *A, basis mode, FILE *out) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            // A->sccs->cl_elems[i][r]);
            if (q == r || (fold && fold->numcl[q] != fold->numcl[r]) ||
                !dgraph_ntrivial_loop(product, sccp, qr)) {
                continue;
            }
            ...
            while (!isempty_dequeue(stack)) {
                ...
                for (uint a = 0; a < A->trans->size_alpha; a++) {
                    ...
                    if (newt != UINT_MAX && news == UINT_MAX) {
                        ...
                        // A->sccs->cl_elems[i][news],
                        // A->sccs->cl_elems[i][newt]);
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            dgraph *thesccp = dgraph_extract(product, sccp, sccp_inv, j);
            parti *foldp =
                mode == BA_ST ? NULL : dgraph_stal_fold(thesccp, NULL, mode);
            // view_dgraph(thesccp);
            dgraph *quad = dgraph_direct_product(thesccp, thesccp);
            parti *sccq  = dtarjan(quad, NULL, false);
            // view_dgraph(quad);
            for (uint h = 0; h < sccq->size_par; h++) {
                ...
                if (dgraph_exists_intersec_path(thesccp, thescc, x1x3, x0, x0x2,
                                                x2, true, NULL)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        ...
                        dgraph_exists_path(thesccp, x0x2, x1x3, true,
                                           &mypattern->words[0]);
                        dgraph_exists_intersec_path(thesccp, thescc, x1x3, x0,
                                                    x0x2, x2, true,
                                                    &mypattern->words[1]);
                        dgraph_exists_path(quad, qstate, qstate, true,
                                           &mypattern->words[2]);
...
bool is_dabpgp_dfa(dfa *A, basis mode, FILE *out) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i0 = 0; i0 < A->sccs->size_par; i0++) {
            ...
            // We extract the SCC
            dgraph *thescc0 =
                dgraph_extract(A->trans, A->sccs, A->sccs_inv, i0);
            ...
            for (uint n = start; n < end; n++) {
                ...
                for (uint j = 0; j < sccp->size_par; j++) {
                    ...
                    uint t1 = t % thescc1->size_graph;
                    if (A->sccs->cl_elems[i0][t0] ==
                            A->sccs->cl_elems[i1][t1] ||
                        !dgraph_ntrivial_loop(product, sccp, t)) {
                        continue;
                    }
                    dgraph *thesccp =
                        dgraph_extract(product, sccp, sccp_inv, j);
                    // view_dgraph(thesccp);
                    dgraph *quad = dgraph_direct_product(thesccp, thesccp);
                    parti *sccq  = dtarjan(quad, NULL, false);
                    // view_dgraph(quad);
                    for (uint h = 0; h < sccq->size_par; h++) {
                        ...
                        uint x0 =
                            A->sccs->cl_elems[i0][sccp->cl_elems[j][x0x2] /
                                                  thescc1->size_graph];
                        uint x2 =
                            A->sccs->cl_elems[i1][sccp->cl_elems[j][x0x2] %
                                                  thescc1->size_graph];
                        uint x1 =
                            A->sccs->cl_elems[i0][sccp->cl_elems[j][x1x3] /
                                                  thescc1->size_graph];
                        uint x3 =
                            A->sccs->cl_elems[i1][sccp->cl_elems[j][x1x3] %
                                                  thescc1->size_graph];

                        if (dgraph_exists_path_alpha(thesccp, x0x2, x1x3, true,
                                                     alpha, NULL) &&
                            dgraph_exists_intersec_path_alpha(
                                thesccp, A->trans, x1x3, x0, x0x2, x2, true,
                                alpha, NULL)) {
                            if (out) {
                                generic_pattern *mypattern =
                                    make_generic_pattern();
                                ...
                                mypattern->types[1]  = PAT_WORD;
                                mypattern->types[2]  = PAT_WORD;
                                dgraph_exists_path_alpha(thesccp, x0x2, x1x3,
                                                         true, alpha,
                                                         &mypattern->words[0]);
                                dgraph_exists_intersec_path_alpha(
                                    thesccp, A->trans, x1x3, x0, x0x2, x2, true,
                                    alpha, &mypattern->words[1]);
                                dgraph_getloop_alpha(quad, sccq, alpha, qstate,
                                                     &mypattern->words[2]);
                                view_pattern(A, mypattern, da_pattern_print);
                                free_generic_pattern(mypattern);
                                CONCLUSION_NEGATIVE(out);
    ...
    for (uint i0 = 0; i0 < A->sccs->size_par; i0++) {
        ...
        for (uint n = start; n < end; n++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                uint t0 = t / thescc1->size_graph;
                uint t1 = t % thescc1->size_graph;
                if (A->sccs->cl_elems[i0][t0] == A->sccs->cl_elems[i1][t1] ||
                    !dgraph_ntrivial_loop(product, sccp, t)) {
                    continue;
                }
                dgraph *thesccp = dgraph_extract(product, sccp, sccp_inv, j);
                ...
                for (uint h = 0; h < sccq->size_par; h++) {
                    ...
                    uint myclass = fold->numcl[0]; // The class of qstate (0 in
                                                   // the numbering of the loop)
                    delete_dgraph(myloop);
                    delete_parti(fold);
                    ...
                    if (dgraph_exists_intersec_path(thesccp, g_fold, x0x2,
                                                    myclass, x1x3, myclass,
                                                    false, NULL) &&
                        dgraph_exists_intersec_path(big, g_fold, big1, myclass,
                                                    big2, myclass, false,
                                                    NULL)) {
                        if (out) {
                            generic_pattern *mypattern = make_generic_pattern();
                            ...
                            mypattern->types[2]        = PAT_LANG;
                            dgraph_exists_intersec_path(
                                thesccp, g_fold, x0x2, myclass, x1x3, myclass,
                                false, &mypattern->words[0]);
                            dgraph_exists_intersec_path(
                                big, g_fold, big1, myclass, big2, myclass,
                                false, &mypattern->words[1]);
                            view_pattern(A, mypattern, da_pattern_print);
                            free_generic_pattern(mypattern);
                            CONCLUSION_NEGATIVE(out);
...
bool is_dabpgp_opti_dfa(dfa *A, basis mode, FILE *out) {
        ...
        for (uint i = 0; i < A->sccs->size_par; i++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                while (!isempty_dequeue(stack)) {
                    ...
                    for (uint a = 0; a < A->trans->size_alpha; a++) {
                        ...
                        uint news = thescc->edges[s][a];
                        uint newt = thescc->edges[t][a];
                        if (newt != UINT_MAX && sccp_alph->numcl[newt] == c &&
                            (news == UINT_MAX || sccp_alph->numcl[news] != c)) {
                            delete_dgraph(product);
                            delete_parti(sccp);
                            delete_dgraph(thescc);
                            free(visited);
                            ...
                            CONCLUSION_NEGATIVE(out);
                            return false;
                        } else if (news != UINT_MAX &&
                                   sccp_alph->numcl[news] == c &&
                                   newt != UINT_MAX &&
                                   sccp_alph->numcl[newt] == c) {
                            // printf("  Following letter %u to (%u,%u)\n", a,
                            // A->sccs->cl_elems[i][news],
                            // A->sccs->cl_elems[i][newt]);
                            uint newst = news * thescc->size_graph + newt;
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                for (uint h = 0; h < sccq->size_par; h++) {
                    ...
                    if (dgraph_exists_path_alpha(thesccp, x0x2, x1x3, true,
                                                 alpha, NULL) &&
                        dgraph_exists_intersec_path_alpha(thesccp, thescc, x1x3,
                                                          x0, x0x2, x2, true,
                                                          alpha, NULL)) {
                        if (out) {
                            generic_pattern *mypattern = make_generic_pattern();
                            ...
                            mypattern->types[1]  = PAT_WORD;
                            mypattern->types[2]  = PAT_WORD;
                            dgraph_exists_path_alpha(thesccp, x0x2, x1x3, true,
                                                     alpha,
                                                     &mypattern->words[0]);
                            dgraph_exists_intersec_path_alpha(
                                thesccp, thescc, x1x3, x0, x0x2, x2, true,
                                alpha, &mypattern->words[1]);
                            dgraph_getloop_alpha(quad, sccq, alpha, qstate,
                                                 &mypattern->words[2]);
                            view_pattern(A, mypattern, da_pattern_print);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            uint jl = foldloop->numcl[0]; // The class of qr (0 in the numbering
                                          // of the loop)
            delete_dgraph(theloop);
            ...
            while (!isempty_dequeue(stack)) {
                ...
                for (uint a = 0; a < A->trans->size_alpha; a++) {
                    ...
                    if (newct != UINT_MAX && mysccs->numcl[newct] == myclass &&
                        (newcs == UINT_MAX ||
                         mysccs->numcl[newcs] != myclass)) {
                        delete_dgraph(product);
                        delete_parti(sccp);
                        delete_dgraph(thescc);
                        free(visited);
                        ...
                        CONCLUSION_NEGATIVE(out);
                        return false;
                    } else if (newcs != UINT_MAX &&
                               mysccs->numcl[newcs] == myclass &&
                               newct != UINT_MAX &&
                               mysccs->numcl[newct] == myclass) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            for (uint h = 0; h < sccq->size_par; h++) {
                ...
                parti *fold    = dgraph_stal_fold(myloop, NULL, mode);
                dgraph *g_fold = dgraph_implement_fold(myloop, NULL, fold);
                uint myclass = fold->numcl[0]; // The class of qstate (0 in the
                                               // numbering of the loop)
                delete_dgraph(myloop);
                delete_parti(fold);
                ...
                if (dgraph_exists_intersec_path(thesccp, g_fold, x0x2, myclass,
                                                x1x3, myclass, false, NULL) &&
                    dgraph_exists_intersec_path(big, g_fold, big1, myclass,
                                                big2, myclass, false, NULL)) {
                    if (out) {
                        generic_pattern *mypattern = make_generic_pattern();
                        mypattern->states[0]       = A->sccs->cl_elems[i][x0];
                        mypattern->states[1]       = A->sccs->cl_elems[i][x1];
...
bool is_dabpgpplus_dfa(dfa *A, basis mode, int *error, FILE *out) {
    ...
    for (uint i = 0; i < bpolorbs->nb_computed; i++) {
        ...
        if (out) {
            ...
            uint r   = 0;
            uint hfl = 0, hfr = 0;
            while (hfl < S->rels->RCL->cl_size[S->rels->RCL->numcl[flsub]] &&
                   hfr < S->rels->LCL->cl_size[S->rels->LCL->numcl[frsub]]) {
                uint rsubl =
                    S->rels->RCL->cl_elems[S->rels->RCL->numcl[flsub]][hfl];
                uint rsubr =
                    S->rels->LCL->cl_elems[S->rels->LCL->numcl[frsub]][hfr];
                if (rsubl < rsubr) {
                    hfl++;
                    continue;
                }
            ...
            uint q   = ONE;
            for (uint qi = 0; qi < A->trans->size_graph; qi++) {
                if (funs[A->trans->size_graph * f + qi] !=
                    funs[A->trans->size_graph * ftf + qi]) {
                    q = qi;
                    break;
                }
            }
        ...
        while (!isempty_dequeue(queue)) {
            ...
            for (uint a = 0; a < A->trans->size_alpha; a++) {
                if (alpha1[a]) {
                    ...
                    uint end =
                        mirror->intervals[r * mirror->size_alpha + a + 1];
                    for (uint i = start; i < end; i++) {
                        rigins_dequeue(mirror->storage[i], queue);
                    }
                }
...
bool is_bpolmod_dfa(dfa *A, FILE *out) {
    ...
    if (out) {
        ...
        if (!is_rtrivialgp_dfa_noprint(A, BA_MOD, cexa)) {
            ...
            dgraph_exists_path(A->trans, cexa->states[1], cexa->states[0],
                               false, &cexa->words[1]);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        // Unary projection of thescc1 (remains deterministic because it is a
        // cycle, this is ensured by the first step) We shall use it to check
        // MOD-separation.
        dgraph *thescc1_unary = dgraph_to_unary(thescc1);

        // We now consider every pair of distinct reachable SCCs from the
        // current one.
        for (uint j = start; j < end; j++) {
            ...
            dgraph *thescc2 = dgraph_extract(A->trans, A->sccs, A->sccs_inv,
                                             A->tclos->storage[j]);
            // printf("Checking SCC2 %u/%u\n", tclos->storage[j] + 1,
            // scca->size_par); view_dgraph(thescc2);
            ...
            for (uint k = j; k < end; k++) {
                ...
                dgraph *thescc3 = dgraph_extract(A->trans, A->sccs, A->sccs_inv,
                                                 A->tclos->storage[k]);
                // printf("Checking SCC3 %u/%u\n", tclos->storage[k] + 1,
                // scca->size_par); view_dgraph(thescc3);

                // We compute the direct product of thescc2 and thescc3 and its
                // SCCs
                ...
                // scc_inter23->size_par); view_dgraph(inter23);

                // For each SCC in the direct product of thescc2 and thescc3
                for (uint h = 0; h < scc_inter23->size_par; h++) {
                    // We extract the scc
                    dgraph *local23       = dgraph_extract(inter23, scc_inter23,
                                                           scc_inter23_inv, h);

                    // Unary projection of local23 (remains deterministic
                    // because we are inside an SCC) Used to check
                    // MOD-separation
                    dgraph *local23_unary = dgraph_to_unary(local23);

                    // We compute the direct product of thescc1_unary and
                    // local23_unary and its SCCs This is the MOD-separation
                    // check
                    dgraph *sepinter =
                        dgraph_direct_product(thescc1_unary, local23_unary);
                    parti *scc_sepinter = dtarjan(sepinter, NULL, false);
                    // printf("Direct product of SCC1 and local23 has %u
                    // SCCs\n", scc_sepinter->size_par); view_dgraph(sepinter);

                    // For each scc and each pair of states
                    for (uint l = 0; l < scc_sepinter->size_par; l++) {
                        // printf(" Checking SCC of the separation intersection
                        // %u/%u\n", l + 1, scc_sepinter->size_par);
                        for (uint li = 0; li < scc_sepinter->cl_size[l]; li++) {
                            uint x0x3x4 = scc_sepinter->cl_elems[l][li];
                            uint x0 =
                                x0x3x4 /
                                local23_unary->size_graph; // Number in thescc1
                            uint x3x4 =
                                x0x3x4 %
                                local23_unary->size_graph; // Number in local23
                            uint x3 = scc_inter23->cl_elems[h][x3x4] /
                                      thescc3->size_graph; // Number in thescc2
                            uint x4 = scc_inter23->cl_elems[h][x3x4] %
                                      thescc3->size_graph; // Number in thescc3

                            if (A->sccs->cl_elems[A->tclos->storage[j]][x3] ==
                                A->sccs->cl_elems[A->tclos->storage[k]][x4]) {
                                // printf("  States in SCC2 and SCC3 are equal,
                                // continuing: %d and %d.\n",
                                // scca->cl_elems[tclos->storage[j]][x3],
                                // scca->cl_elems[tclos->storage[k]][x4]);
                                continue;
                            }
                            // printf(" State in x0 SCC1: %d.\n",
                            // scca->cl_elems[i][x0]); printf("  States in x3
                            // SCC2 and x4 SCC3 are distinct: %d and %d.\n",
                            // scca->cl_elems[tclos->storage[j]][x3],
                            // scca->cl_elems[tclos->storage[k]][x4]);
                            for (uint lj = 0; lj < scc_sepinter->cl_size[l];
                                 lj++) {
                                uint x1x2x5 = scc_sepinter->cl_elems[l][lj];
                                uint x1 = x1x2x5 /
                                          local23_unary
                                              ->size_graph; // Number in thescc1
                                uint x2x5 =
                                    x1x2x5 %
                                    local23_unary
                                        ->size_graph; // Number in local23
                                uint x2 =
                                    scc_inter23->cl_elems[h][x2x5] /
                                    thescc3->size_graph; // Number in thescc2
                                uint x5 =
                                    scc_inter23->cl_elems[h][x2x5] %
                                    thescc3->size_graph; // Number in thescc3
                                // printf(" State in x1 SCC1: %d.\n",
                                // scca->cl_elems[i][x1]); printf("  States in
                                // x2 SCC2 and x5 SCC3 are distinct: %d and
                                // %d.\n",
                                // scca->cl_elems[tclos->storage[j]][x2],
                                // scca->cl_elems[tclos->storage[k]][x5]);
                                // printf("   Checking paths... %d and %d\n",
                                // x3x4, x2x5); view_dgraph(inter23);
                                if (dgraph_exists_intersec_path(
                                        A->trans, local23,
                                        A->sccs->cl_elems[i][x0], x3x4,
                                        A->sccs->cl_elems[A->tclos->storage[j]]
                                                         [x2],
                                        x2x5, false, NULL) &&
                                    dgraph_exists_intersec_path(
                                        A->trans, local23,
                                        A->sccs->cl_elems[i][x1], x2x5,
                                        A->sccs->cl_elems[A->tclos->storage[k]]
                                                         [x4],
                                        x3x4, false, NULL)) {
                                    if (out) {
                                        generic_pattern *mypattern =
                                            make_generic_pattern();
                                        mypattern->states[0] =
                                            A->sccs->cl_elems[i][x0];
                                        mypattern->states[1] =
                                            A->sccs->cl_elems[i][x1];
                                        mypattern->states[2] =
                                            A->sccs
                                                ->cl_elems[A->tclos->storage[j]]
                                                          [x2];
                                        mypattern->states[3] =
                                            A->sccs
                                                ->cl_elems[A->tclos->storage[j]]
                                        ...
                                        mypattern->types[3] = PAT_WORD;
                                        dgraph_exists_path(
                                            A->trans, A->sccs->cl_elems[i][x1],
                                            A->sccs->cl_elems[i][x0], false,
                                            &mypattern->words[1]);
                                        dgraph_exists_intersec_path(
                                            A->trans, local23,
                                            A->sccs->cl_elems[i][x1], x2x5,
...
bool is_bpolamt_dfa(dfa *A, FILE *out) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        // A contradiction cannot arise if there are less than two reachable
        // SCCs from the current one.
        if (end - start < 1) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            dgraph *thescc2 = dgraph_extract(A->trans, A->sccs, A->sccs_inv,
                                             A->tclos->storage[j]);
            // printf("Checking SCC2 %u/%u\n", A->tclos->storage[j] + 1,
            // A->sccs->size_par); view_dgraph(thescc2);
            ...
            for (uint k = j; k < end; k++) {
                ...
                dgraph *thescc3 = dgraph_extract(A->trans, A->sccs, A->sccs_inv,
                                                 A->tclos->storage[k]);
                // printf("Checking SCC3 %u/%u\n", A->tclos->storage[k] + 1,
                // A->sccs->size_par); view_dgraph(thescc3);
                ...
                // For each SCC in the direct product of thescc2 and thescc3
                for (uint h = 0; h < scc_inter23->size_par; h++) {
                    uint u2  = scc_inter23->cl_elems[h][0] /
                               thescc3->size_graph; // Number in thescc2
                    uint u3  = scc_inter23->cl_elems[h][0] %
                               thescc3->size_graph; // Number in thescc3
                    ...
                    num_span_forest *spanlocal =
                        compute_span_forest(local23, NULL, NULL);

                    fmpz_mat_t MAT;
                    build_hnf_matrix_two(thescc1, local23, spanone, spanlocal,
                                         0, 0, MAT);
                    ...
                    for (uint x0 = 0; x0 < thescc1->size_graph; x0++) {
                        ...
                        for (uint x1 = 0; x1 < thescc1->size_graph; x1++) {
                            ...
                            for (uint x3x4 = 0; x3x4 < local23->size_graph;
                                 x3x4++) {
                                uint x3 =
                                    scc_inter23->cl_elems[h][x3x4] /
                                    thescc3->size_graph; // Number in thescc2
                                uint x4 =
                                    scc_inter23->cl_elems[h][x3x4] %
                                    thescc3->size_graph; // Number in thescc3
                                uint x3i =
                                    A->sccs->cl_elems[A->tclos->storage[j]][x3];
                                uint x4i =
                                    A->sccs->cl_elems[A->tclos->storage[k]][x4];
                                for (uint x2x5 = 0; x2x5 < local23->size_graph;
                                     x2x5++) {
                                    uint x2 =
                                        scc_inter23->cl_elems[h][x2x5] /
                                        thescc3
                                            ->size_graph; // Number in thescc2
                                    uint x5 =
                                        scc_inter23->cl_elems[h][x2x5] %
                                        thescc3
                                            ->size_graph; // Number in thescc3
                                    uint x2i =
                                        A->sccs->cl_elems[A->tclos->storage[j]]
                                                         [x2];
                                    uint x5i =
                                    ...
                                    for (uint a = 0; a < A->trans->size_alpha;
                                         a++) {
                                        target[a] =
                                            (spanone->span_forest[x1][a] -
                                             spanone->span_forest[x0][a]) -
                                            (spanlocal->span_forest[x2x5][a] -
                                             spanlocal->span_forest[x3x4][a]);
                                    }
                                    // printf(" Checking states %d, %d, %d, %d,
                                    // %d, %d\n", x0i, x1i, x2i, x3i, x4i, x5i);

                                    if (solve_system_amt(MAT, target,
                                                         fmpz_mat_nrows(MAT),
                                                         fmpz_mat_ncols(MAT)) &&
                                        dgraph_exists_intersec_path(
                                            local23, A->trans, x3x4, x0i, x2x5,
                                            x2i, false, NULL) &&
                                        dgraph_exists_intersec_path(
                                            local23, A->trans, x2x5, x1i, x3x4,
                                            x4i, false, NULL)) {
                                        if (out) {
                                            generic_pattern *mypattern =
                                                make_generic_pattern();
                                            ...
                                            mypattern->types[3]  = PAT_WORD;
                                            dgraph_exists_path(
                                                A->trans, x1i, x0i, false,
                                                &mypattern->words[1]);
                                            dgraph_exists_intersec_path(
                                                A->trans, local23, x1i, x2x5,
                                                x4i, x3x4, false,
                                                &mypattern->words[3]);
                                            view_pattern(A, mypattern,
                                                         knast_pattern_print);
...
bool is_bpoldd_dfa(dfa *A, FILE *out) {
    ...
    // No contradiction can arise at this point if there are less than three
    // SCCs.
    dfa_compute_sccs(A);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            // end - start);
            if (A->tclos->storage[j] == i ||
                !dgraph_ntrivial_scc(A->trans, A->sccs, A->tclos->storage[j])) {
            ...
            for (uint k = start; k < end; k++) {
                ...
                // + 1, end - start);
                if (A->tclos->storage[k] == i || A->tclos->storage[k] == j ||
                    !dgraph_ntrivial_scc(A->trans, A->sccs,
                                         A->tclos->storage[k])) {
                    // printf("      Second reachable SCC is trivial,
                    // continuing\n");
                    continue;
                }
                ...
                for (uint h = 0; h < scc_inter123->size_par; h++) {
                    ...
                    for (uint g = h; g < scc_inter123->size_par; g++) {
                        ...
                        if (scc_inter23->numcl[x3x4] !=
                            scc_inter23->numcl[x2x5]) {
                            continue;
                        }
                        ...
                        // printf("        Considering states (%u,%u,%u)\n",
                        // A->sccs->cl_elems[i][x1],
                        // A->sccs->cl_elems[A->tclos->storage[j]][x2],
                        // A->sccs->cl_elems[A->tclos->storage[k]][x5]);

                        if (dgraph_exists_intersec_path(
                                A->trans, inter23, A->sccs->cl_elems[i][x0],
                                x3x4,
                                A->sccs->cl_elems[A->tclos->storage[j]][x2],
                                x2x5, false, NULL) &&
                            dgraph_exists_intersec_path(
                                A->trans, inter23, A->sccs->cl_elems[i][x1],
                                x2x5,
                                A->sccs->cl_elems[A->tclos->storage[k]][x4],
                                x3x4, false, NULL)) {
                            if (out) {
                                ...
                                dgraph_exists_intersec_path(
                                    ...
                                    x3x4,
                                    A->sccs->cl_elems[A->tclos->storage[j]][x2],
                                    x2x5, true, &mypattern->words[2]);
                                dgraph_exists_intersec_path(
                                    A->trans, inter23, A->sccs->cl_elems[i][x1],
                                    x2x5,
                                    A->sccs->cl_elems[A->tclos->storage[k]][x4],
                                    x3x4, true, &mypattern->words[3]);
                                dgraph_exists_path(inter123, x0x3x4, x0x3x4,
                                                   true, &mypattern->words[4]);
                                dgraph_exists_path(inter123, x1x2x5, x1x2x5,
                                                   true, &mypattern->words[5]);
...
bool is_bpolamtplus_dfa(dfa *A, FILE *out) {
    ...
    if (out) {
        ...
        if (!is_rtrivialgpplus_dfa_noprint(A, BA_AMT, cexa)) {
            ...
            cexa->types[4]  = PAT_WORD;
            cexa->types[5]  = PAT_WORD;
            dgraph_exists_path(A->trans, cexa->states[1], cexa->states[0], true,
                               &cexa->words[1]);
            cexa->words[3] = copy_word(cexa->words[2]);
            cexa->words[4] = copy_word(cexa->words[2]);
            cexa->words[5] = copy_word(cexa->words[2]);
                ...
                for (uint h = 0; h < scc_inter23->size_par; h++) {
                    ...
                    for (uint c1 = 0; c1 < scc_inter123->size_par; c1++) {
                        ...
                        for (uint c2 = 0; c2 < scc_inter123->size_par; c2++) {
                            if (!dgraph_ntrivial_scc(inter123, scc_inter123,
                                                     c2)) {
                                continue;
                            }
                            ...
                            uint x2     = scc_inter23->cl_elems[h][x2x5] /
                                          thescc3->size_graph;
                            uint x5     = scc_inter23->cl_elems[h][x2x5] %
                                          thescc3->size_graph;
                            uint x2o =
                                A->sccs->cl_elems[A->tclos->storage[j]][x2];
                            uint x5o =
                            ...
                            // printf(" Considering states (%u,%u,%u)\n", x1o,
                            // x2o, x5o); printf(" Checking states %u, %u, %u,
                            ...
                            for (uint a = 0; a < A->trans->size_alpha; a++) {
                                target[a] = (spanone->span_forest[x1][a] -
                                             spanone->span_forest[x0][a]) -
                                            (spanlocal->span_forest[x2x5][a] -
                                             spanlocal->span_forest[x3x4][a]);
                                // printf("  Target[%u] = %d\n", a, target[a]);
                            ...
                            if (solve_system_amt(MAT, target,
                                                 fmpz_mat_nrows(MAT),
                                                 fmpz_mat_ncols(MAT)) &&
                                dgraph_exists_intersec_path(local23, A->trans,
                                                            x3x4, x0o, x2x5,
                                                            x2o, false, NULL) &&
                                dgraph_exists_intersec_path(local23, A->trans,
                                                            x2x5, x1o, x3x4,
                                ...
                                if (out) {
                                    ...
                                    mypattern->types[4]  = PAT_WORD;
                                    mypattern->types[5]  = PAT_WORD;
                                    dgraph_exists_path(A->trans, x1o, x0o,
                                                       false,
                                                       &mypattern->words[1]);
                                    dgraph_exists_intersec_path(
                                        A->trans, local23, x1o, x2x5, x4o, x3x4,
                                        false, &mypattern->words[3]);
                                    dgraph_exists_path(inter123, x0x3x4, x0x3x4,
                                                       true,
                                                       &mypattern->words[4]);
                                    dgraph_exists_path(inter123, x1x2x5, x1x2x5,
...
bool is_bpolgpplus_dfa(dfa *A, bool grp, FILE *out) {
    MAKE_PATTERN_INTRO_STRICT(
        grp ? BPOL_PATTERN_GR_PLUS : BPOL_PATTERN_MOD_PLUS, out);
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        // Folding of thescc1 with inverse transitions (used to check
        // GR-separation)
        parti *fold1    = dgraph_stal_fold(thescc1, NULL, grp ? BA_GR : BA_MOD);
        dgraph *fold1_g = grp ? shrink_grp(thescc1, fold1, NULL)
                              : shrink_mod(thescc1, fold1, NULL);
        // num_span_forest *spanone = compute_span_forest(thescc1, NULL, NULL);
        ...
        for (uint j = start; j < end; j++) {
            ...
            for (uint k = start; k < end; k++) {
                ...
                for (uint h = 0; h < scc_inter23->size_par; h++) {
                    ...
                    // Folding of the scc with inverse transitions (used to
                    // check GR-separation)
                    parti *foldlocal =
                        dgraph_stal_fold(local23, NULL, grp ? BA_GR : BA_MOD);
                    dgraph *foldlocal_g =
                        grp ? shrink_grp(local23, foldlocal, NULL)
                            : shrink_mod(local23, foldlocal, NULL);
                    ...
                    for (uint c1 = 0; c1 < scc_inter123->size_par; c1++) {
                        ...
                        for (uint c2 = 0; c2 < scc_inter123->size_par; c2++) {
                            ...
                            if (dgraph_exists_intersec_path(
                                    foldlocal_g, fold1_g,
                                    foldlocal->numcl[x3x4], fold1->numcl[x0],
                                    foldlocal->numcl[x2x5], fold1->numcl[x1],
                                    false, NULL) &&
                                dgraph_exists_intersec_path(local23, A->trans,
                                                            x3x4, x0o, x2x5,
                                                            x2o, false, NULL) &&
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = start; j < end; j++) {
            ...
            for (uint k = j; k < end; k++) {
                // printf("    Checking second reachable SCC %u/%u\n", k - start
                // + 1, end - start);
                if (!dgraph_ntrivial_scc(A->trans, A->sccs,
                                         A->tclos->storage[k])) {
                    // printf("      Second reachable SCC is trivial,
                    // continuing\n");
                    continue;
                ...
                parti *scc_inter23 = dtarjan(inter23, NULL, false);
                parti *fold_inter23 =
                    dgraph_stal_fold(inter23, scc_inter23, mode);
                dgraph *inter123    = dgraph_direct_product(thescc1, inter23);
                // view_dgraph(inter123);
                parti *scc_inter123 = dtarjan(inter123, NULL, false);

                parti *fold_inter123 =
                    dgraph_stal_fold(inter123, scc_inter123, mode);
                ...
                for (uint h = 0; h < fold_inter123->size_par; h++) {
                    ...
                    for (uint g = 0; g < fold_inter123->size_par;
                         g++) // for (uint g = 0; g < scc_inter123->size_par;
                              // g++)
                    {
                        uint x1x2x5 = fold_inter123->cl_elems[g][0];
                        if (!dgraph_ntrivial_loop(inter123, scc_inter123,
                                                  x1x2x5)) {
                            continue;
                        }
                        ...
                        //// printf("PrePart 2: Considering states
                        ///(%u,%u,%u)\n", A->sccs->cl_elems[i][x1],
                        /// A->sccs->cl_elems[A->tclos->storage[j]][x2],
                        /// A->sccs->cl_elems[A->tclos->storage[k]][x5]);
                        if (fold_inter23->numcl[x3x4] !=
                                fold_inter23->numcl[x2x5] ||
                            fold_thescc1->numcl[x0] !=
                                fold_thescc1->numcl[x1]) {
                            continue;
                        }
            ...
            for (uint k = start; k < end; k++) {
                ...
                for (uint h = 0; h < scc_inter123->size_par; h++) {
                    ...
                    for (uint g = h; g < scc_inter123->size_par; g++) {
                        ...
                        dgraph_common_alph_loop(inter123, scc_inter123,
                                                scc_inter123_inv, x0x3x4,
                                                x1x2x5, alpha);
                        ...
                        if (dgraph_exists_path_alpha(thescc1, x0, x1, false,
                                                     alpha, NULL) &&
                            dgraph_exists_path_alpha(thescc1, x1, x0, false,
                                                     alpha, NULL) &&
                            dgraph_exists_intersec_path_alpha(
                                A->trans, inter23, A->sccs->cl_elems[i][x0],
                                x3x4,
                                A->sccs->cl_elems[A->tclos->storage[j]][x2],
                                x2x5, false, alpha, NULL) &&
                            dgraph_exists_intersec_path_alpha(
                                A->trans, inter23, A->sccs->cl_elems[i][x1],
                                x2x5,
                                A->sccs->cl_elems[A->tclos->storage[k]][x4],
                                x3x4, false, alpha, NULL)) {
                            if (out) {
                                generic_pattern *mypattern =
                                    make_generic_pattern();
                                mypattern->states[0] = A->sccs->cl_elems[i][x0];
                                ...
                                mypattern->types[4] = PAT_WORD;
                                mypattern->types[5] = PAT_WORD;
                                dgraph_exists_path_alpha(thescc1, x0, x1, false,
                                                         alpha,
                                                         &mypattern->words[0]);
                                dgraph_exists_path_alpha(thescc1, x1, x0, false,
                                                         alpha,
                                ...
                                dgraph_exists_intersec_path_alpha(
                                    ...
                                    x3x4,
                                    A->sccs->cl_elems[A->tclos->storage[j]][x2],
                                    x2x5, true, alpha, &mypattern->words[2]);
                                dgraph_exists_intersec_path_alpha(
                                    A->trans, inter23, A->sccs->cl_elems[i][x1],
                                    x2x5,
                                    A->sccs->cl_elems[A->tclos->storage[k]][x4],
                                    x3x4, true, alpha, &mypattern->words[3]);
                                dgraph_getloop_alpha(inter123, scc_inter123,
                                                     alpha, x0x3x4,
                                                     &mypattern->words[4]);
                                dgraph_getloop_alpha(inter123, scc_inter123,
                                                     alpha, x1x2x5,
...
bool is_upolgpplus_dfa(dfa *A, basis mode, FILE *out) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint k = 0; k < sccp->size_par; k++) {
            ...
            uint r  = qr % thescc->size_graph;
            if (q != r &&
                (!thefold || thefold->numcl[q] == thefold->numcl[r]) &&
                dgraph_ntrivial_loop(product, sccp, qr)) {
                if (out) {
                    generic_pattern *mypattern = make_generic_pattern();
                    mypattern->states[0]       = A->sccs->cl_elems[i][q];
                    ...
                    if (mode == BA_ST) {
                        mypattern->vars[0]  = 'u';
                        mypattern->types[0] = PAT_WORD;
                        dgraph_exists_path(thescc, q, r, true,
                                           &mypattern->words[0]);
                    } else {
                        mypattern->vars[0]  = 'U';
                        mypattern->types[0] = PAT_LANG;
    ...
    // At the point, if a counterexample exists, one for UPol(GRP) exists as
    // well.
    dfa_compute_tclos(A);
...
bool is_upolbp2gp_dfa(dfa *A, basis mode, FILE *out) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i1 = 0; i1 < A->sccs->size_par; i1++) {
            // If the SCC is trivial (one state and no loop), it cannot yield a
            // counterexample and we skip it.
            if (!dgraph_ntrivial_scc(A->trans, A->sccs, i1)) {
                continue;
            }
            // We extract the SCC
            ...
            for (uint n = start; n < end; n++) {
                ...
                if (!dgraph_ntrivial_scc(A->trans, A->sccs, i2) ||
                    (i1 == i2 && A->sccs->cl_size[i1] < 2)) {
                    continue;
                }
                dgraph *thescc2 =
                    dgraph_extract(A->trans, A->sccs, A->sccs_inv, i2);
                // printf("  Checking reachable SCC %u/%u\n", n - start + 1, end
                // - start); view_dgraph(thescc2);
                ...
                for (uint j = 0; j < sccp->size_par; j++) {
                    ...
                    if (A->sccs->cl_elems[i1][t1] ==
                            A->sccs->cl_elems[i2][t2] ||
                        !dgraph_ntrivial_loop(product, sccp, t)) {
                        continue;
                    }
                    dgraph *thesccp =
                    ...
                    for (uint h = 0; h < sccq->size_par; h++) {
                        ...
                        if (dgraph_exists_path_alpha(thesccp, x0x2, x1x3, true,
                                                     alpha, NULL) &&
                            dgraph_exists_intersec_path(
                                thesccp, A->trans, x1x3,
                                A->sccs->cl_elems[i1][x0], x0x2,
                                A->sccs->cl_elems[i2][x2], true, NULL)) {
                            if (out) {
                                generic_pattern *mypattern =
                                    make_generic_pattern();
                                mypattern->states[0] =
                                    A->sccs->cl_elems[i1][x0];
                                mypattern->states[1] =
                                    A->sccs->cl_elems[i1][x1];
                                mypattern->states[2] =
                                    A->sccs->cl_elems[i2][x2];
                                mypattern->states[3] =
                                    A->sccs->cl_elems[i2][x3];
                                mypattern->vars[0]  = 'u';
                                mypattern->vars[1]  = 'v';
                                mypattern->vars[2]  = 'w';
                                mypattern->types[0] = PAT_WORD;
    ...
    for (uint i1 = 0; i1 < A->sccs->size_par; i1++) {
        ...
        for (uint n = start; n < end; n++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                uint t1 = t / thescc2->size_graph;
                uint t2 = t % thescc2->size_graph;
                if (A->sccs->cl_elems[i1][t1] == A->sccs->cl_elems[i2][t2] ||
                    !dgraph_ntrivial_loop(product, sccp, t)) {
                    continue;
                }
                dgraph *thesccp = dgraph_extract(product, sccp, sccp_inv, j);
                ...
                for (uint h = 0; h < sccq->size_par; h++) {
                    ...
                    dgraph *myfold_g =
                        dgraph_implement_fold(myscc, NULL, myfold);
                    uint sl = myfold->numcl[0];
                    delete_dgraph(myscc);
                    delete_parti(myfold);

                    if (dgraph_exists_intersec_path(thesccp, myfold_g, x0x2, sl,
                                                    x1x3, sl, true, NULL) &&
                        dgraph_exists_intersec_path(
                            thesccp, A->trans, x1x3, A->sccs->cl_elems[i1][x0],
                            x0x2, A->sccs->cl_elems[i2][x2], true, NULL)) {
                        if (out) {
                            generic_pattern *mypattern = make_generic_pattern();
                            ...
                            mypattern->types[1]  = PAT_WORD;
                            mypattern->types[2]  = PAT_LANG;
                            dgraph_exists_intersec_path(thesccp, myfold_g, x0x2,
                                                        sl, x1x3, sl, true,
                                                        &mypattern->words[0]);
                            dgraph_exists_intersec_path(
                                thesccp, A->trans, x1x3,
...
bool is_upolbp2gp_opti_dfa(dfa *A, basis mode) {
    ...
    if (mode == BA_ST) {
        ...
        for (uint i = 0; i < A->sccs->size_par; i++) {
            ...
            for (uint j = 0; j < sccp->size_par; j++) {
                ...
                bool rtoq =
                    dgraph_exists_path_alpha(thescc, r, q, false, alpha, NULL);
                bool qtor =
                    dgraph_exists_path_alpha(thescc, q, r, false, alpha, NULL);
                if (!rtoq && !qtor) {
                    continue;
                }
                // printf("Found path\n");
                ...
                while (!isempty_dequeue(stack)) {
                    ...
                    for (uint a = 0; a < A->trans->size_alpha; a++) {
                        ...
                        // UINT_MAX ? news : scca->cl_elems[i][news], newt ==
                        // UINT_MAX ? newt : scca->cl_elems[i][newt]);
                        if (qtor && newt != UINT_MAX && news == UINT_MAX) {
                            ...
                            // printf("  Following letter %u to (%u,%u)\n", a,
                            // scca->cl_elems[i][news],
                            // scca->cl_elems[i][newt]);
                            uint newst = news * thescc->size_graph + newt;
                            rigins_dequeue(newst, stack);
                        }
                    }
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            bool rtoq = dgraph_exists_intersec_path(
                thefold_g, thescc, thefold->numcl[0], r, thefold->numcl[0], q,
                false, NULL);
            bool qtor = dgraph_exists_intersec_path(
                thefold_g, thescc, thefold->numcl[0], q, thefold->numcl[0], r,
                false, NULL);
            ...
            while (!isempty_dequeue(stack)) {
                ...
                for (uint a = 0; a < A->trans->size_alpha; a++) {
                    ...
                    uint newt = thescc->edges[t][a];
                    // printf("  On letter %u to (%u,%u)\n", a, news == UINT_MAX
                    // ? news : scca->cl_elems[i][news], newt == UINT_MAX ? newt
                    // : scca->cl_elems[i][newt]);
                    if (qtor && newt != UINT_MAX && news == UINT_MAX) {
                        delete_dgraph(product);
                        delete_parti(sccp);
                        ...
                        // printf("  Following letter %u to (%u,%u)\n", a,
                        // scca->cl_elems[i][news], scca->cl_elems[i][newt]);
                        uint newst = news * thescc->size_graph + newt;
                        rigins_dequeue(newst, stack);
                    }
                }
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            for (uint h = 0; h < sccq->size_par; h++) {
                ...
                if (dgraph_exists_intersec_path(thesccp, myfold_g, x0x2, sl,
                                                x1x3, sl, true, NULL) &&
                    dgraph_exists_intersec_path(thesccp, thescc, x1x3, x0, x0x2,
                                                x2, true, NULL)) {
                    delete_dgraph(myfold_g);
                    free(sccq_inv);
                    delete_dgraph(product);
...
bool is_upolbp2gpplus_opti_dfa(dfa *A, basis mode) {
    ...
    for (uint i = 0; i < A->sccs->size_par; i++) {
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            bool rtoq = dgraph_exists_intersec_path_pgplus(
                thescc, theloop, r, 0, q, 0, false, theloopprod, scc_loopprod,
                thefold);
            bool qtor = dgraph_exists_intersec_path_pgplus(
                thescc, theloop, q, 0, r, 0, false, theloopprod, scc_loopprod,
                thefold);
            if (!rtoq && !qtor) {
                continue;
            }
        ...
        for (uint j = 0; j < sccp->size_par; j++) {
            ...
            for (uint h = 0; h < sccq->size_par; h++) {
                ...
                if (!dgraph_exists_intersec_path(thesccp, thescc, x1x3, x0,
                                                 x0x2, x2, true, NULL)) {
                    continue;
                }
                ...
                dgraph *thelooptwo = dgraph_direct_product(theloop, theloop);
                dgraph *theloopprod =
                    dgraph_direct_product(thesccp, thelooptwo);
                parti *scc_loopprod = dtarjan(theloopprod, NULL, false);
                delete_dgraph(thelooptwo);

                if (dgraph_exists_intersec_path_pgplus(
                        thesccp, theloop, x0x2, 0, x1x3, 0, false, theloopprod,
                        scc_loopprod, thefold)) {
                    delete_dgraph(product);
                    delete_parti(sccp);
                    free(sccp_inv);
                    delete_dgraph(thescc);
