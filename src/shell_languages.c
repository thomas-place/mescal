...
object* object_automata_new(char* name, nfa* A) {
	...
	MALLOC(object, 1);
	object->name = strdup(name);
	object->type = AUTOMATA;
	object->parent = -1;
	automata* aut;
	MALLOC(aut, 1);
	aut->nfa = A;
	object->theob.aut = aut;
	return object;
}
...
void object_free(object* object) {
	...
	{
	case AUTOMATA:
		if (object->theob.aut == NULL) {
			free(object);
			return;
		}
		delete_nfa(object->theob.aut->nfa);
		free(object->theob.aut);
		break;
	case MORPHISM:
		if (object->theob.mor == NULL) {
			free(object);
			return;
		}
		free_cayley(object->theob.mor->cayley);
		if (object->theob.mor->green)
		{
			delete_green(object->theob.mor->green);
		}
		if (object->theob.mor->ddorbs)
		{
			delete_orbits(object->theob.mor->ddorbs, true);
		}
		if (object->theob.mor->mporbs)
		{
			delete_orbits(object->theob.mor->mporbs, false);
		}
		if (object->theob.mor->atorbs)
		{
			delete_orbits(object->theob.mor->atorbs, true);
		}
		if (object->theob.mor->bpmorbs)
		{
			delete_orbits(object->theob.mor->bpmorbs, true);
		}
		if (object->theob.mor->bpgorbs)
		{
			delete_orbits(object->theob.mor->bpgorbs, true);
		}
		if (object->theob.mor->bpmporbs)
		{
			delete_orbits(object->theob.mor->bpmporbs, true);
		}
		if (object->theob.mor->bpddorbs)
		{
			delete_orbits(object->theob.mor->bpddorbs, true);
		}
		if (object->theob.mor->mker)
		{
			delete_green_sub(object->theob.mor->mker);
		}
		if (object->theob.mor->gker)
		{
			delete_green_sub(object->theob.mor->gker);
		}
		free(object->theob.mor);
		break;
	case LANGUAGE:
		if (object->theob.lan == NULL) {
			free(object);
			return;
		}
		// Si le langage est spécifié par une expression,
		// on la libère.
		if (object->theob.lan->type == SPE_REG) {
			if (object->theob.lan->spe.reg)
			{
				reg_free(object->theob.lan->spe.reg);
			}
		}
		// Si le langage est spécifié par un NFA, on enlève
		// le flag qui protège ce NFA.
		else if (object->theob.lan->type == SPE_NFA) {
			objects[object->theob.lan->spe.nfa]->parent = -1;
		}
		// Si le langage est associèe à un minimal, on enlève
		// le flag qui protège ce minimal.
		if (object->theob.lan->minauto != -1) {
			objects[object->theob.lan->minauto]->parent = -1;
		}
		// Si le langage est associèe à un morphism syntaxique
		// le flag qui le protège.
		if (object->theob.lan->syntmor != -1) {
			objects[object->theob.lan->syntmor]->parent = -1;
		}
		free(object->theob.lan);
		break;
...
int object_add_language_reg(char* name, regexp* regexp)
{
	...
	if (i != -1 && objects[i]->parent == -1)
	{
		...
		object_free(objects[i]);
		objects[i] = object_language_new_reg(name, regexp);
		return i;
	}
	else if (i != -1 && objects[i]->parent != -1)
	{
		...
		objects[nb_objects++] = object_language_new_reg(name, regexp);
		return nb_objects - 1;
	}
...
int shell_compute_syntac(int i) {
	...
	language* lang = objects[i]->theob.lan;
	// On commence par calculer l'automate minimal du langage (si ce n'est pas déjà fait)
	...
	if (lang->syntmor == -1) {
		...
		lang->syntmor = object_add_morphism(newname, dfa_to_right_cayley(mini->nfa));
...
void shell_compute_bpddorbs(morphism* mor) {
	...
	// Si le neutre a un antécédent non-vide, les BPol(DD)-orbites sont exactement
...
void shell_compute_bpmporbs(morphism* mor) {
	...
	// exactement les BPol(MOD)-orbites (il suffit donc de calculer ces dernières)
	if (cayley_neutstrict(mor->cayley, mor->green)) {
