...
// Calcule la plus petite partition contenant celle prise en entrée et compatible avec les transitions du graphe
...
static void cong_fold(p_dgraph G, p_ufind uf)
{
    ...
    // On ne va remplir les listes que pour les représentants dans le union-find original
    ...
    for (uint q = 0; q < G->size_graph; q++) // Remplissage des listes avec les transitions d'origine
    {
        uint r = find_ufind(q, uf); // On ne remplit que les listes des représentant dans la partition initiale
        // Pour chaque lettre
        for (uint a = 0; a < G->size_alpha; a++)
        {
    ...
    while (!isempty_vertices(tofold))
    {
        ...
        {
            if (ltrans[s][a]->size > 1) // Si il y a 2 arêtes sortantes étiquetées par a
            {
                // On prend les représentants des destinations des deux premières arêtes qu'on va fusionner (si ça n'a pas été fait avant)
                uint r = find_ufind(ltrans[s][a]->lsent->next->val, uf);
                uint t = find_ufind(ltrans[s][a]->lsent->next->next->val, uf);
                deletecell_dlist(ltrans[s][a], ltrans[s][a]->lsent->next); // On supprime la première des deux edges
                if (r != t)                                                // Si les deux sommets adjacents n'avaient pas encore étés fusionnés
                {
                    union_ufind(r, t, uf);                   // On effectue la fusion
                    for (uint b = 0; b < G->size_alpha; b++) // On concatène leur listes de sommets adjacents
                    {
                        concat_dlist(ltrans[r][b], ltrans[t][b]);
                        if (find_ufind(r, uf) == r) // On ne garde que la liste du nouveau représentant de la classe
                        {
                            free(ltrans[t][b]);
                            ltrans[t][b] = NULL;
                        }
                    ...
                    rigins_vertices(find_ufind(r, uf), tofold); // On va devoir éventuelement traiter la nouvelle classe
...
p_ufind iden_bpolmod_mono(p_cayley M, p_green G)
{
    ...
    // Gestion du cas pariculier où 1 a un antécédent non-vide (dans ce cas on retourne simplement les J-classes)
    if (M->neutlet)
    {
                    ...
                    if (M->idem_array[qr] && G->JCL->numcl[qr] == G->JCL->numcl[q])
                    {
                        for (uint k = 0; k < size_vertices(FOLD->cl[d]); k++)
                        {
                            ...
                            if (G->regular_array[s])
                            {
                                ...
                                for (uint l = 0; l < size_vertices(G->JCL->cl[G->JCL->numcl[s]]); l++)
                                {
                                    ...
                                    {
                                        union_ufind(cayley_mult(M, qr, st), cayley_mult_gen(M, 4, qr, q, t, st), uf);
                                    }
                                }
                            }
                        }
...
p_ufind iden_knast_mono(p_cayley M, p_green G)
{
    ...
    // Gestion du cas partitculier où 1 a un antécédent non-vide (dans ce cas, on retourne simplement la partition en J-classes)
    ...
    // Tableau qui mémorise les idempotents e déjà traités (il arrive que certains soient traités simultanément)
    ...
    for (uint i = 1; i < size_vertices(M->idem_list); i++)
    {
        ...
        if (!edone[e])
        {
            ...
            // L'équation étant symmétrique, il suffit de considérer les f tels que e <= f

            // Tableau qui mémorise les idempotents f déjà traités pour ce e (il arrive que certains soient traités simultanément)
            ...
            // Calcul des ensembles eM et Me (on ne garde que les éléments réguliers dans ces ensembles)
            // On marque les idempotents e' dans eM et Me: ils sont traités automatiquement en même temps que e
            ...
            // On commence par considérer le cas f = e: on doit vérifier (esete)^w = (esete)^w ese et (esete)^w = ete(esete)^w pour tous s,t
            // On calcule eMe
            ...
            for (uint k = 0; k < size_vertices(eMe) - 1; k++)
            {
                ...
                for (uint l = k; l < size_vertices(eMe); l++)
                {
                    ...
                    if (G->JCL->numcl[ese] == G->JCL->numcl[ete])
                    {
                        ...
                        // Il suffit de considérer les cas où esete J ese et esete est idempotent
            ...
            for (uint j = i + 1; j < size_vertices(M->idem_list); j++)
            {
                ...
                if (!fdone[f])
                {
                    ...
                    // fprintf(out, "\n");
                    // Calcul des ensembles fM et Mf (on ne garde que les éléments réguliers dans ces ensembles)
                    // On marque les idempotents f' dans fM et Mf: ils sont traités automatiquement en même temps que f
                    ...
                    for (uint k = 0; k < size_vertices(eMf); k++)
                    {
                        ...
                        for (uint l = 0; l < size_vertices(fMe); l++)
                        {
                            ...
                            if (G->JCL->numcl[eqf] == G->JCL->numcl[fre])
                            {
                                ...
                                // Il suffit de considérer les cas où eqfre J eqf et eqfre est idempotent
                                if (M->idem_array[eqfre] && G->JCL->numcl[eqf] == G->JCL->numcl[eqfre])
                                {
                                    ...
                                    for (uint m = 0; m < size_vertices(eMf); m++)
                                    {
                                        ...
                                        for (uint n = 0; n < size_vertices(fMe); n++)
                                        {
                                            ...
                                            if (G->JCL->numcl[esf] == G->JCL->numcl[fte])
                                            {
                                                ...
                                                // fprintf(out, "\n");
                                                // Il suffit de considérer les cas où esfte J esf et esfte est idempotent
                                                if (M->idem_array[esfte] && G->JCL->numcl[esf] == G->JCL->numcl[esfte])
                                                {
                                                    union_ufind(cayley_mult(M, eqfre, esfte), cayley_mult_gen(M, 4, eqfre, eqf, fte, esfte), uf);
                                                }
                                            }
                                        }
                                    }
...
p_ufind iden_qknast_mono(p_cayley M, p_green G, p_green_sub mker)
{
    ...
    // Tableau qui marque les éléments du MOD-kernel qui sont réguliers dans celui-ci
    ...
    for (uint i = 1; i < size_vertices(M->idem_list); i++)
    {
        ...
        if (!edone[e])
        {
            ...
            for (uint k = 0; k < size_vertices(eMe) - 1; k++)
            {
                ...
                {
                    uint ete = lefread_vertices(eMe, l);
                    // Il suffit de considérer les éléments ete tels que ese J ete (dans le MOD-kernel)
                    if (mker->rels->JCL->numcl[mker->mono_to_sub[ese]] == mker->rels->JCL->numcl[mker->mono_to_sub[ete]])
                    {
                        uint esete = cayley_mult(M, ese, ete);
                        // Il suffit de considérer les cas où esete J ese  (dans le MOD-kernel) et esete est idempotent
                        if (M->idem_array[esete] && mker->rels->JCL->numcl[mker->mono_to_sub[ese]] == mker->rels->JCL->numcl[mker->mono_to_sub[esete]])
                        {
            ...
            for (uint j = i + 1; j < size_vertices(M->idem_list); j++)
            {
                ...
                if (!fdone[f])
                {
                    // Calcul des ensembles fM et Mf (on ne garde que les éléments réguliers du MOD-kernel dans ces ensembles)
                    // On marque les idempotents f' dans fM et Mf: ils sont traités automatiquement en même temps que f
                    ...
                    for (uint k = 0; k < size_vertices(eMf); k++)
                    {
                        ...
                        {
                            uint fre = lefread_vertices(fMe, l);
                            // Il suffit de considérer les cas où eqf J fre (dans le MOD-kernel)
                            if (mker->rels->JCL->numcl[mker->mono_to_sub[eqf]] == mker->rels->JCL->numcl[mker->mono_to_sub[fre]])
                            {
                                uint eqfre = cayley_mult(M, eqf, fre);
                                // Il suffit de considérer les cas où eqfre J eqf et eqfre est idempotent
                                if (M->idem_array[eqfre] && mker->rels->JCL->numcl[mker->mono_to_sub[eqf]] == mker->rels->JCL->numcl[mker->mono_to_sub[eqfre]])
                                {
                                    // Boucle sur les éléments esf
                                    for (uint m = 0; m < size_vertices(eMf); m++)
                                    {
                                        ...
                                        {
                                            uint fte = lefread_vertices(fMe, n);
                                            // Il suffit de considérer les cas où eqf J fre (dans le MOD-kenrel)
                                            if (mker->rels->JCL->numcl[mker->mono_to_sub[esf]] == mker->rels->JCL->numcl[mker->mono_to_sub[fte]])
                                            {
                                                uint esfte = cayley_mult(M, esf, fte);
                                                // Il suffit de considérer les cas où esfte J esf et esfte est idempotent
                                                if (M->idem_array[esfte] && mker->rels->JCL->numcl[mker->mono_to_sub[esf]] == mker->rels->JCL->numcl[mker->mono_to_sub[esfte]])
                                                {
...
// Retourne l'équivalence obtenue en identifiant qui devraient être égauc selon l'équation de MPol(C)
                ...
                if (i != j)
                {
                    ...
                    for (uint k = 0; k < size_vertices(sM); k++)
                    {
                        ...
                        for (uint l = 0; l < size_vertices(Mt); l++)
                        {
                            ...
                            // printf(".\n");
                            // Il suffit de considérer le cas où rt est idempotent
                            ...
                            if (sqsrt != sqtrt)
                            {
                                ...
                                union_ufind(cayley_mult_gen(M, 3, sq, s, rt), cayley_mult_gen(M, 3, sq, t, rt), theuf);
                            }
                        }
                        delete_vertices(Mt);
                    }
...
// Calcul du niveau dans la hiérarchie MPol. On passe un "prototype" de la congruence canonique C en entrée.
// (la plus petit congruence contenant la paertition doit être la congruence canonique C)
// On suppose que l'input est dans UPol(C) (ne termine pas dans le cas contraire)
