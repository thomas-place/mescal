...
static bool extract_bool_nosink(int i)
{
    if (objects[i].type != DAUTOMATON)
    {
        return false;
    }
    return is_nosink_dfa(objects[i].obj_dfa);
}

static bool extract_bool_noselfloops(int i)
{
    if (objects[i].type != DAUTOMATON)
    {
        return false;
    }
    return is_noselfloops_dfa(objects[i].obj_dfa);
}
...
bool (*bool_extractors[FILTER_BOOLSIZE])(int i) = {
    ...
    extract_bool_nosink,     // FILTER_NOSINK
    extract_bool_noselfloops // FILTER_NOSELFLOOPS
};
...
int shell_retrieve_filters(com_parameters *pars, filters_info *info)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        case KY_NOSMALLCOUNTER:
            ...
            break;
        case KY_NOSINK:
            info->used_boolean[FILTER_NOSINK] = true;
            info->sign_boolean[FILTER_NOSINK] = not;
            if (com_nbparams(param->params) > 0)
            {
                shell_nopar_filter_error(param->main->string);
                return -2;
            ...
            break;
        case KY_NOSELFLOOPS:
            info->used_boolean[FILTER_NOSELFLOOPS] = true;
            info->sign_boolean[FILTER_NOSELFLOOPS] = not;
            if (com_nbparams(param->params) > 0)
            {
                shell_nopar_filter_error(param->main->string);
                return -2;
