...
bool shell_morprop_monojsat(int j, char *name, bool co, FILE *out)
{
    if (out)
    {
        fprintf(out, "#### Checking if the %s M satisfies the inequation 1 %s s for all s ∊ M.\n", name, co ? "≥" : "≤");
    }
    if (is_jsat_mono(objects[j].mor->obj, co, get_counter(out)))
    {
        if (out)
        {
...
bool shell_morprop_monoejsat(int j, char *name, bool co, FILE *out)
{
    if (out)
    {
        fprintf(out, "#### Checking if the %s M satisfies the inequation 1 %s e for all e ∊ E(M).\n", name, co ? "≥" : "≤");
    }
    if (is_ejsat_mono(objects[j].mor->obj, co, get_counter(out)))
    {
        if (out)
        {
            fprintf(out, "#### The inequation is satisfied.\n");
...
bool shell_morprop_kerjsat(int j, kernel_type type, char *ker, char *name, bool co, FILE *out)
{

    if (out)
    {
        fprintf(out, "#### Checking if the %s-kernel N of the %s satisfies\n", ker, name);
        fprintf(out, "     the inequation 1 %s s for all s ∊ N.\n", co ? "≥" : "≤");
    }
    if (is_jsat_subsemi(shell_compute_ker(j, type, LV_GREG), ONE, co, get_counter(out)))
    {
        if (out)
        {
            fprintf(out, "#### The inequation is satisfied.\n");
...
bool shell_morprop_orbjsat(int j, orbits_type type, char *orbs, char *name, bool co, FILE *out)
{

    if (out)
    {
        fprintf(out, "#### Checking if every %s-orbit Mₑ for the %s satisfies\n", orbs, name);
        fprintf(out, "     the inequation e %s s for all s ∊ Mₑ.\n", co ? "≥" : "≤");
    }
    if (is_jsat_orbmono(shell_compute_orbits(j, type, LV_GREG), co, get_counter(out)))
    {
        if (out)
        {
