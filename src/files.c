...
json_object *files_nfa_to_json(nfa *A)
{
    ...
    // Number of states in the automaton.
    json_object_object_add(root, "NB_STATES", json_object_new_int(A->trans->size_graph));

    // Number of letters in the automaton.
    json_object_object_add(root, "NB_LETTERS", json_object_new_int(A->nb_letters));

    // Inverse transitions
    json_object_object_add(root, "INVERSE", json_object_new_boolean(A->inverse));

    // Epsilon transitions
    json_object_object_add(root, "EPSILON", json_object_new_boolean(A->epsilon));

    // Alphabet
    json_object *alphabet = json_object_new_array_ext(A->nb_letters);
    char buffer[2];
    for (uint i = 0; i < A->nb_letters; i++)
    {
    ...
    json_object_object_add(root, "ALPHABET", alphabet);

    // List of transitions (we start by counting the number of transitions)
    uint num_trans = A->trans->size_edges;
    ...
    for (uint q = 0; q < A->trans->size_graph; q++)
    {
        for (uint a = 0; a < A->trans->size_alpha; a++)
        {
            ...
            for (uint i = start; i < end; i++)
            {
                ...
                json_object_array_add(onetrans, json_object_new_int(A->trans->storage[i]));
                json_object_array_add(trans, onetrans);
            }
        }
    }
    json_object_object_add(root, "TRANSITIONS", trans);

    // Liste des états initiaux
    json_object *initials = json_object_new_array_ext(A->nb_initials);
    for (uint i = 0; i < A->nb_initials; i++)
    {
        json_object_array_add(initials, json_object_new_int(A->initials[i]));
    }

    json_object_object_add(root, "INITIALS", initials);
    ...
    json_object_object_add(root, "FINALS", finals);
...
json_object *files_dfa_to_json(dfa *A)
{
    ...
    // Nombre de lettres de l'automate
    json_object_object_add(root, "NB_LETTERS", json_object_new_int(A->trans->size_alpha));

    // Inverse transitions
    json_object_object_add(root, "INVERSE", json_object_new_boolean(false));

    // Epsilon transitions
    json_object_object_add(root, "EPSILON", json_object_new_boolean(false));
...
void *files_json_to_automaton(json_object *root, bool *is_dfa)
{
    ...
    json_object *jsize_graph;
    if (!json_object_object_get_ex(root, "NB_STATES", &jsize_graph) || json_object_get_type(jsize_graph) != json_type_int)
    {
        fprintf(stderr, "Error: cannot read the size of the automaton.\n");
        return NULL;
    }
    ...
    if (!json_object_object_get_ex(root, "NB_LETTERS", &jsize_alpha) || json_object_get_type(jsize_alpha) != json_type_int)
    {
    ...
    // Retrieiving the epsilon flag
    json_object *jepsilon;
    if (!json_object_object_get_ex(root, "EPSILON", &jepsilon) || json_object_get_type(jepsilon) != json_type_boolean)
    {
        fprintf(stderr, "Error: cannot read the epsilon flag.\n");
        return NULL;
    }
    bool epsilon = json_object_get_boolean(jepsilon);

    // Retrieiving the inverse flag
    json_object *jinverse;
    if (!json_object_object_get_ex(root, "INVERSE", &jinverse) || json_object_get_type(jinverse) != json_type_boolean)
    {
        fprintf(stderr, "Error: cannot read the inverse flag.\n");
        return NULL;
    }
    bool inverse = json_object_get_boolean(jinverse);

    // The number of labels in the graph
    uint nblabels = (size_alpha << (inverse ? 1 : 0)) + (epsilon ? 1 : 0);

    // Retrieving the alphabet
    json_object *jalph;
    if (!json_object_object_get_ex(root, "ALPHABET", &jalph) || json_object_get_type(jalph) != json_type_array)
    {
        fprintf(stderr, "Error: cannot read the alphabet.\n");
        return NULL;
    }
    if (json_object_array_length(jalph) != size_alpha)
    {
        fprintf(stderr, "Error: invalid alphabet size.\n");
        return NULL;
    }
    ...
    json_object *jtrans;
    if (!json_object_object_get_ex(root, "TRANSITIONS", &jtrans) || json_object_get_type(jtrans) != json_type_array)
    {
        fprintf(stderr, "Error: cannot read the list of transitions.\n");
        free(alphabet);
        return NULL;
    ...
    for (uint i = 0; i < len; i++)
    {
        ...
        json_object *ja = json_object_array_get_idx(onetrans, 1);
        json_object *jr = json_object_array_get_idx(onetrans, 2);
        if (json_object_get_type(jq) != json_type_int || json_object_get_type(jr) != json_type_int || json_object_get_type(ja) != json_type_int)
        {
            fprintf(stderr, "Error: cannot read the list of transitions.\n");
            free(alphabet);
            free(trans);
        ...
        trans[id].from = json_object_get_int(jq);
        trans[id].to = json_object_get_int(jr);
        trans[id].lab = json_object_get_int(ja);

        if (trans[id].lab >= nblabels)
        {
            fprintf(stderr, "Error: incorrect letter used in the transitions.\n");
            free(alphabet);
            free(trans);
    ...
    // Récupération de la liste des états initiaux
    json_object *jini;
    if (!json_object_object_get_ex(root, "INITIALS", &jini) || json_object_get_type(jini) != json_type_array)
    {
        fprintf(stderr, "Error: cannot read the list of initial states.\n");
        free(alphabet);
        free(trans);
    ...
    // Récupération de la liste des états finaux
    json_object *jfin;
    if (!json_object_object_get_ex(root, "FINALS", &jfin) || json_object_get_type(jfin) != json_type_array)
    {
        fprintf(stderr, "Error: cannot read the list of final states.\n");
        free(alphabet);
        free(trans);
    ...
    thenfa->nb_letters = size_alpha;
    thenfa->inverse = inverse;
    thenfa->epsilon = epsilon;
    thenfa->nb_initials = inilen;
    thenfa->nb_finals = finlen;
    MALLOC(thenfa->initials, inilen);
    MALLOC(thenfa->finals, finlen);
    for (uint i = 0; i < inilen; i++)
    {
        thenfa->initials[i] = initials[i];
    }
    for (uint i = 0; i < finlen; i++)
    {
        thenfa->finals[i] = finals[i];
    }
    free(initials);
    free(finals);
    thenfa->trans = edge_list_to_lgraph(trans, len, size_graph, nblabels);
    free(trans);
