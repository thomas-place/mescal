    ...
    for (uint q = 0; q < A->trans->size_graph;q++) {
        for (uint a = 0; a < A->trans->size_alpha;a++) {
            for (uint i = 0; i < size_vertices(A->trans->edges[q][a]);i++) {
                ...
                json_object_array_add(onetrans, json_object_new_int(lefread_vertices(A->trans->edges[q][a], i)));
    ...
    if (A->etrans != NULL) {
        ...
        for (uint q = 0; q < A->trans->size_graph;q++) {
            for (uint i = 0; i < size_vertices(A->etrans->edges[q]);i++) {
                ...
                json_object_array_add(onetrans, json_object_new_int(q));
                json_object_array_add(onetrans, json_object_new_int(lefread_vertices(A->etrans->edges[q], i)));
...
json_object* files_lang_to_json(language* thelang) {
    ...
    if (thelang->type == SPE_NFA) {
        ...
        json_object_object_add(root, "AUTO", files_auto_to_json(objects[thelang->spe.nfa]->theob.aut));
    }
    return root;
}
...
p_nfa files_json_to_nfa(json_object* root) {
    ...
    if (!json_object_object_get_ex(root, "N_ALPH", &jsize_alpha) || json_object_get_type(jsize_alpha) != json_type_int) {
    ...
    for (uint i = 0; i < len; i++)
    {
        ...
        else if (json_object_get_type(ja) == json_type_string && json_object_get_string_len(ja) == 1) {
    ...
    if (json_object_object_get_ex(root, "L_ETRA", &jetrans)) {
        ...
        for (uint i = 0; i < len; i++)
        {
            ...
            if (q < size_graph && r < size_graph) {
                ...
                fprintf(stderr, "Error: incorrect state used in the list of epsilon transitions.\n");
                delete_nfa(A);
                return NULL;
        ...
        for (uint i = 0; i < len; i++)
        {
            ...
            if (json_object_get_type(ja) == json_type_int) {
                ...
                if ('a' <= a && a <= 'z') {
                    ...
                    fprintf(stderr, "Error: incorrect letter used in the list of inverse transitions.\n");
                    delete_nfa(A);
                    return NULL;
                }
            ...
            if (q < size_graph && r < size_graph && a < size_alpha) {
                ...
                fprintf(stderr, "Error: incorrect letter or state used in the list of inverse transitions.\n");
                delete_nfa(A);
                return NULL;
...
static void files_read_object_aux(json_object* root, char* varname) {
    ...
    if (!json_object_object_get_ex(root, "TYPE", &object_type) || json_object_get_type(object_type) != json_type_string) {
        fprintf(stderr, "Error: cannot read the type of the object.\n");
        return;

    }
    const char* thetype = json_object_get_string(object_type);
    ...
    if (strcmp(thetype, "LANG") == 0) {
        ...
        if (!json_object_object_get_ex(root, "SPEC", &lang_spe) || json_object_get_type(lang_spe) != json_type_string) {
        ...
        if (strcmp(thespec, "REXP") == 0) {
            ...
            if (!json_object_object_get_ex(root, "REXP", &theregexp) || json_object_get_type(theregexp) != json_type_string) {
                fprintf(stderr, "Error: cannot read the regular expression specifying the language.\n");
                return;
            }
        ...
        if (strcmp(thespec, "AUTO") == 0) {
            json_object* theauto;
            if (!json_object_object_get_ex(root, "AUTO", &theauto) || json_object_get_type(theauto) != json_type_object) {
