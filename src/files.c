...
json_object* files_auto_to_json(nfa* A) {
    ...
    for (uint i = 0; i < A->trans->size_alpha; i++) {
        ...
        if (A->alphabet[i].num < 0) {
            ...
            json_object* letter = json_object_new_array_ext(2);
            json_object_array_add(letter, json_object_new_string(buffer));
            json_object_array_add(letter, json_object_new_int(A->alphabet[i].num));
            json_object_array_add(alphabet, letter);
        }
    }
...
void files_save_object(object* theobj, char* filename) {
    ...
    case REGEXP:
        ...
        break;
    case AUTOMATON:
        root = files_auto_to_json(theobj->nfa);
        break;
    default:
        return;
        break;
...
static void files_read_object_aux(json_object* root, char* varname) {
    ...
    if (strcmp(thetype, "AUTO") == 0) {
        ...
        object_add_automaton(varname, A);
        return;
    }
    ...
    if (strcmp(thetype, "REXP") == 0) {
        ...
        object_add_regexp(varname, myexp);
        return;
    }
}
    ...
    for (int i = 0; i < nb_objects; i++) {
        if (objects[i]->parent == -1) {
            ...
            if (objects[i]->type == AUTOMATON) {
                json_object_array_add(oneobj, files_auto_to_json(objects[i]->nfa));
            }
