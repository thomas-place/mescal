...
uint lefread_dequeue(dequeue* p, uint i)
{
    ...
    {
        fprintf(stderr, "Error, the dequeue is not large enough.\nSize: %d.\nIndex read: %d\n", size_dequeue(p), i);
        exit(EXIT_FAILURE);
    }
...
static bool mem_dequeue_sorted_aux(uint e, dequeue* p, uint i, uint j, uint* ind)
{
    if (j <= i)
    {
        return false;
...
void merge_sorted_dequeue(dequeue* p1, dequeue* p2)
{
    ...
    while (i < n1 || j < n2)
    {
        if (i == n1 || (j < n2 && lefread_dequeue(p2, j) < lefread_dequeue(p1, 0)))
        {
            if ((i == 0 && j == 0) || lefread_dequeue(p2, j) != rigread_dequeue(p1, 0))
            {
        ...
        {
            if ((i == 0 && j == 0) || lefread_dequeue(p1, 0) != rigread_dequeue(p1, 0))
            {
...
void merge_array_sorted_dequeue(dequeue* out, dequeue** array, uint size)
{
    ...
    while (j < max_size)
    {
        ...
        {
            if (index[i] < size_dequeue(array[i]) && lefread_dequeue(array[i], index[i]) < min)
            {
